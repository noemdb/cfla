...
PROMPT;
    ...
    /**
     * Genera contenido HTML para la diapositiva (sección) actual usando IA.
     * Similar a generateSectionContent() pero produce HTML semántico
...
PROMPT;
    ...
    /**
     * Genera código HTML con diagrama Mermaid para la diapositiva actual
     * usando el contenido de la sección y el contexto de la lección.
     * Reutiliza la lógica existente de generateEmbedCard() pero inyecta
    ...
    public function generateSlideDiagram(): void
    {
        ...
        $sectionTitle = $sectionData['title'] ?? 'Sección';
        $activity = $this->selectedActivity;
        $pevaluacion = $activity?->pevaluacion;
        ...
        $sectionContentPreview = ! empty($sectionContents)
            ? $sectionContents
            : '(La sección no tiene contenido aún.)';

        $systemPrompt = <<<'PROMPT'
Eres un Staff Engineer frontend especializado en diagramas Mermaid.js y Tailwind CSS.
Genera código HTML autónomo para un diagrama Mermaid enmarcado en un card simple.
...
5. Elige el tipo de diagrama según el contenido (graph, sequenceDiagram, mindmap, etc.)
6. Sin scripts externos. Sin wrappers markdown. Solo HTML puro.
7. El diagrama debe reflejar fielmente el contenido pedagógico.
...
CONDICIÓN NO NEGOCIABLE — RESPONSIVE DESIGN:
...
- Cada bloque de DESARROLLO: mínimo 100 palabras (3-5 párrafos) — coherente con el umbral mínimo de la validación automática (100 palabras)
...
PROMPT;
    ...
    public function saveStep2(): void
    {
        ...
        //  Variables que se pueblan dentro de la transacción (para notificación)
        $visibleResourceIds = [];
        $visibleLinkIds = [];
        $visibleEmbedIds = [];

        // ─── Persistir todo dentro de una transacción ────────────
        // Protege el borrado/recreación de contenidos: si algo falla a
    ...
    private function renderHierarchicalInfografia(array $nodoRaiz, callable $getColor, callable $getTextColor, callable $getIcon, int $niveles): string
    {
        ...
        $drawNode = function ($node, $x, $y, $level, $parentX = null, $parentY = null) use (&$drawNode, $getColor, $getTextColor, $niveles, &$svg) {
    ...
    private function renderRadialInfografia(array $nodoRaiz, callable $getColor, callable $getTextColor, callable $getIcon, int $niveles): string
    {
        ...
        $drawLevel = function ($nodes, $radius, $level) use (&$drawLevel, $getColor, $getTextColor, $niveles, &$svg) {
    ...
    private function renderFlowInfografia(array $nodoRaiz, callable $getColor, callable $getTextColor, callable $getIcon, int $niveles): string
    {
        ...
        $drawFlowNode = function ($node, $x, $y, $level, $parentIndex = null, $isDecision = false) use (&$drawFlowNode, $getColor, $getTextColor, $niveles, &$svg, $nodeWidth, $nodeHeight, $levelSpacing, &$currentY) {
            if ($level > $niveles || ! is_array($node)) {
                return null;
            }
    ...
    /**
     * Valida que la respuesta del servicio de IA tenga la estructura esperada.
     */
    public function validateInfografiaResponse(array $response): bool
    {
        // Validar que la respuesta tenga la estructura esperada
        if (! isset($response['estructura']) || ! is_array($response['estructura'])) {
            return false;
        }

        // Validar que cada nivel tenga los campos requeridos
        foreach ($response['estructura'] as $level => $items) {
