...
class QuestionByPensum extends Component
{
    ...
    public function render()
    {
        ...
        // Grados anidados a pestudio (pestudio->grados)
        ...
        if ($this->pestudioId) {
            ...
            $gradosOptions = Grado::whereIn('id', Pensum::whereIn('id', $pensumIdsWithQuestions)->pluck('grado_id'))
                ->orderBy('pestudio_id')->orderBy('order')->get(['id', 'name', 'code', 'pestudio_id']);
        }

        // Secciones anidadas a grado (grado->seccions) — pestudio->grados->seccions
        ...
        if ($this->gradoId) {
            ...
            $gradoIds = Grado::where('pestudio_id', $this->pestudioId)->pluck('id');
            $seccionsOptions = Seccion::whereIn('grado_id', $gradoIds)->orderBy('name')->get(['id', 'name', 'grado_id']);
        } else {
            $gradoIds = Pensum::whereIn('id', $pensumIdsWithQuestions)->pluck('grado_id')->unique();
            $seccionsOptions = Seccion::whereIn('grado_id', $gradoIds)->orderBy('name')->get(['id', 'name', 'grado_id']);
        }

        // Pensums filtrados por pestudio→grado→seccion (anidados)
        ...
        if ($selectedPensum) {
            ...
            // Pevaluaciones de ese pensum → agrupar por grupo_estable_id (respetando filtro de sección)
