...
trait DeepSeekTrait
{
    ...
    /**
     * Start the competition creation process with AI (DeepSeek)
     */
    public function dsCreateCompetition()
    {
        $this->close();
        $this->modeCreatorGeminiCompetition = true; // Still using same mode variable to avoid changing too much UI logic
        $this->activities = collect();
    }

    /**
     * Generate competition data using AI (DeepSeek)
     */
    public function dsGenerateCompetition()
    {
        $this->validate([
        ...
        $context = $this->dsBuildContextFromActivities();

        if ($context) {
            $prompt = Competition::getPrompt($context);
            $arrData = $this->dsGetDeepSeekResponse($prompt);

            if ($this->dsValidateCompetitionData($arrData)) {
                $this->dsSaveGeneratedCompetition($arrData, $context);

                $title = '¡Excelente, buen trabajo! ';
                $html = 'Se ha generado los datos principales para una nueva competición (vía DeepSeek)';
    ...
    /**
     * Start the debate creation process with AI (DeepSeek)
     */
    public function dsCreateDebate($id)
    {
        $this->close();
    ...
    /**
     * Generate debate and questions using AI (DeepSeek)
     */
    public function dsGenerateAiDebate($id)
    {
        $this->validate([
        ...
        $prompt = $this->dsBuildDebatePrompt($competition);
        $arrData = $this->dsGetDeepSeekResponse($prompt);

        if ($this->dsValidateDebateData($arrData)) {
            $this->dsSaveGeneratedDebate($competition, $arrData);

            $title = '¡Excelente, buen trabajo! ';
            $html = 'Se ha generado y registrado un debate con 5 preguntas y sus opciones (vía DeepSeek).';
            $this->showSwal($title, $html);
            $this->close();
            $this->competition_id = $competition->id;
            $this->competition = $competition;
        } else {
            $this->dsShowErrorSwal();
        }
    }

    private function dsGetDeepSeekResponse($prompt)
    {
        $deepseek = app(DeepSeekService::class);
        $payload = [
            'model' => 'deepseek-chat',
            'messages' => [
                ['role' => 'system', 'content' => 'Eres un experto en pedagogía integradora y metodología STEM. Responde siempre envolviendo el JSON en bloques de código markdown: ```json ... ```'],
        ...
        try {
            $response = $deepseek->generateText($payload);

            // Extracting content from DeepSeek response
            $content = null;
            ...
            Log::error('DeepSeek Generation Error: ' . $e->getMessage());
        }

        return null;
    }

    private function dsBuildContextFromActivities()
    {
        $context = "";
        $i = 0;
    ...
    private function dsBuildDebatePrompt($competition)
    {
        $perspectives = $this->dsGetSelectedPerspectives();
        $cognitive = $this->dsGetSelectedCognitiveStyles();
    ...
    private function dsValidateCompetitionData($data)
    {
        return is_array($data) &&
            isset($data['name'], $data['description'], $data['motive']);
    }

    private function dsValidateDebateData($data)
    {
        return is_array($data) &&
    ...
    private function dsSaveGeneratedCompetition($data, $context)
    {
        $this->resetModel();
    ...
    private function dsSaveGeneratedDebate($competition, $data)
    {
    ...
    private function dsGetSelectedPerspectives()
    {
        $list = [];
    ...
    private function dsGetSelectedCognitiveStyles()
    {
        $list = [];
    ...
    private function dsShowErrorSwal()
    {
