...
namespace App\Http\Controllers\Leader\PDF;
...
class CompetitionController extends Controller
{
    public function batch($grado_id = null)
    {
        ...
        $view = View::make('plannings.competitions.pdf.batch',
            compact('questions','grado','institucion','autoridad1','autoridad2','fecha'))
            ->render();
    ...
    public function cards($pensum_id, $category)
    {
        ...
        if (array_key_exists($category, $list_category)) {
            $grado = $pensum->grado;
            $questions = DebateQuestion::where('pensum_id',$pensum_id)->orderBy('weighting')->where('category',$category)->get();
            ...
            $view = View::make('plannings.competitions.pdf.cards',
                compact('questions','grado','category','institucion','autoridad1','autoridad2','fecha'))
                ->render();
