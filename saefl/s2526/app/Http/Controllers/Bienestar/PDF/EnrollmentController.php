...
class EnrollmentController extends Controller
{
    public function ficha($student_record_id){
        ...
        $view =  View::make('bienestars.enrollments.pdf.ficha',compact($compact))->render();
    ...
    public function batch(Request $request)
    {
        ...
        if (count($request->all())>0) {
            $estudiants =
                Estudiant::select('estudiants.*')
                    ->join('enrollments', 'estudiants.id', '=', 'enrollments.estudiant_id')
        ...
        $view =  View::make('bienestars.enrollments.pdf.batch',compact($compact))->render();
