...
use App\Models\app\Bienestar\Traits\EnrollmentTrait;
...
class Enrollment extends Model
{
    ...
    {
        $url = ($this->photo) ? asset('storage/enrollments/'.$this->photo) : null;
        $url = (Storage::disk('enrollments')->exists($this->photo)) ? $url : null ; //dd($path,$url);
        return $url;
    }
    ...
    public static function representants()
    {
        $representants = Enrollment::select('enrollments.*')
        ->groupBy('enrollments.ci_representant')
        ->get();
    ...
    {
        return Enrollment::select('enrollments.*')->where('ci_representant',$this->ci_representant)->get();
    }

    public function getFullInscripcionAttribute()
    {
        return $this->grado->name;
    }
    ...
    {
        return $this->name_representant .' '.$this->lastname_representant;
    }

    public function getEmailAttribute()
    {
        return $this->email_representant;
    }
    ...
    public static function getEnrollmentsFormaly()
    {
    ...
    public function scopeEnrollmentsFormaly($query)
    {
        $query =  $query->join('estudiants', 'estudiants.ci_estudiant', '=', 'enrollments.ci_estudiant')
            ->join('inscripcions', 'estudiants.id', '=', 'inscripcions.estudiant_id')
