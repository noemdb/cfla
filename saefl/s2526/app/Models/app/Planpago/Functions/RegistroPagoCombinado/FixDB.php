...
trait FixDB
{
    public static function irregular_pay($start=1,$size=100)
    {
        ...
        foreach ($registro_pago_combinados as $registro_pago_combinado) {
            ...
            $diferencia = round( ($total_debitos - $total_creditos));
    ...
    public function getStatusIrregularAttribute()
    {
        ...
        $diferencia = round( ($total_debitado - $total_acreditado));

        return ($diferencia <> 0) ? true:false;
    }

    public static function irregular_pay_representant($representant_id)
    {

        $datas = collect([]);

        $registro_pago_combinados = RegistroPagoCombinado::where('representant_id',$representant_id)->get();
