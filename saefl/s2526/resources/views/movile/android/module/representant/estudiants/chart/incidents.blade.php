@php
    $chart = [
        'range'=>'1',
        'id_chart'=>'incidents_id_'.Auth::user()->id,
        'urlapi'=>route('movile.incidents.chart'),
        'tipo'=>'line',
        'limit'=>6,
        'legend'=>true,
        'y_axes'=>'Cantidad',
...
@section('scripts')
    @parent <script> requestData('{{ $chart['range'] }}','{{ $chart['id_chart'] }}','{{ $chart['urlapi'] }}','{{ $chart['tipo'] }}','{{ $chart['limit'] }}'); </script>
@endsection

@component('profesors.elements.card.panel')
    @slot('class', 'success')
    @slot('panelControls', 'true')
    @slot('id', $chart['id_chart'])
    @slot('header', 'Distribución de los registros de incidencias')
    @slot('iconTitle', $icon_menus['chartbar'])
    @slot('body')
        @component('profesors.elements.canvas.chart')
