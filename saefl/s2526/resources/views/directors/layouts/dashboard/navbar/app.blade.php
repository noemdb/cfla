...
<div class="collapse navbar-collapse" id="navbarTopAdministracion">
  <ul class="navbar-nav px-3">
    ...
    <li class="nav-item nav-link pl-1">
      <a class="btn btn-{{ (Request::is('*home*')) ? 'light':'success' }}" title="Inicio" href="{{ route('directors.home') }}" >
        <i class="{{ $icon_menus['inicio'] }} fa-1x text-{{ (Request::is('*home*')) ? 'success':'light' }}"></i>
      </a>
    </li>

    <li class="nav-item nav-link pl-1">
      <a class="btn btn-{{ (Request::is('*pollmains*')) ? 'light':'success' }} disabled" title="Procesos de Consultas" href="{{ route('directors.pollmains.index') }}">
        <i class="{{ $icon_menus['pollmain'] ?? null }} fa-1x text-{{ (Request::is('*pollmains*')) ? 'success':'light' }}"></i>
      </a>
    </li>

    <li class="nav-item nav-link pl-1">
      <a class="btn btn-{{ (Request::is('*financial*')) ? 'light':'success' }} disabled" title="Indicadores Financieros" href="{{ route('directors.administrations.financial') }}">
        <i class="{{ $icon_menus['administracion'] }} fa-1x text-{{ (Request::is('*financial*')) ? 'success':'light' }}"></i>
      </a>
    </li>
    <li class="nav-item nav-link pl-1">
      <a class="btn btn-{{ (Request::is('*controls*')) ? 'light':'success' }}" title="Indicadores de Control de Estudio" href="{{ route('directors.controls.performance') }}">
        <i class="{{ $icon_menus['control_estudio'] }} fa-1x text-{{ (Request::is('*controls*')) ? 'success':'light' }}"></i>
      </a>
    </li>
    <li class="nav-item nav-link pl-1">
      <a class="btn btn-{{ (Request::is('*audits*')) ? 'light':'success' }}" title="Indicadores de Control de Estudio" href="{{ route('directors.audits.usages') }}">
        <i class="{{ $icon_menus['chartline'] }} fa-1x text-{{ (Request::is('*audits*')) ? 'success':'light' }}"></i>
      </a>
    </li>

    <li class="nav-item nav-link pl-1">
        @include('directors.layouts.dashboard.navbar.partials.usermenu')
    </li>
