...
@endphp
    ...
    <table width="100%" class="table table-striped table-sm table-hover small p-1" id="table-data-default">
        ...
        @foreach($pevaluacions as $pevaluacion)
            ...
            @php $pensum = $pevaluacion->pensum; $grado = $pevaluacion->pensum->grado; $pensum = $pevaluacion->pensum; $grupo_estable = $pevaluacion->grupo_estable;@endphp
            ...
            class="table-{{(empty($pevaluacion->administrativa->id)) ? 'default':'success'}}">
                ...
                <td class="{{ $class_action ?? '' }}" id="btn-action-{{ $pevaluacion->id }}">
                    <div class="btn-group">
                        ...
                        <a title="Registrar Evaluaciones" class="btn {{$class_btn ?? ''}}" href="{{route('profesors.evaluacions.create',$pevaluacion->id)}}" role="button">
                        ...
                        <a title="Registro de Notas" class="btn {{$class_btn ?? ''}} {{$disabled ?? ''}} btn-primary" href="{{route('profesors.boletins.carga',['pevaluacion_id'=>$pevaluacion->id])}}" role="button">
                            <i class="{{ $icon_menus['notas'] ?? ''}} fa-1x"></i>
                        </a>
                        {{-- <a title="Carga de Notas XLS" class="btn {{$class_btn ?? ''}} {{$disabled ?? ''}} btn-success" href="{{route('profesors.boletins.carga.xls',['pevaluacion_id'=>$pevaluacion->id])}}" role="button">
                            <i class="{{ $icon_menus['notas'] ?? ''}} fa-1x"></i>
                        </a> --}}
