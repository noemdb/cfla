@php
    ...
    $class_estudiant = '';
    $class_gsemail = 'd-none d-xl-table-cell';
    $class_ci = '';
    $class_pensum = 'nosort';
    $class_action = 'nosort';

    $disabled_preclosing = $lapso->status_preclosing ? false : true;
    $disabled_pevalucion = $pevaluacion->status_carga_notas ? false : true;

    $disabled = $disabled_preclosing || $disabled_pevalucion ? ' disabled ' : null;
...
{!! Form::open([
    'route' => 'profesors.boletins.store',
    'method' => 'POST',
    'id' => 'form-nota',
...
<fieldset id="table-fieldset" {{ $disabled ?? null }} data-id="fieldset">
    {{-- <fieldset id="fieldset" {{ ($pevaluacion->status_carga_notas) ? null:'disabled' }}> --}}
    ...
    <table width="100%" class="table table-striped table-hover table-sm small p-1" id="table-data-default">
        <thead>
            <tr>
                ...
                <th class="{{ $class_gsemail }}" title="Email para las clases virtuales">EmailCV</th>
                    ...
                    @if ($pevaluacion)
                        ...
                        @if (!empty($pevaluacion->evaluacions->first()))
                            @foreach ($pevaluacion->evaluacions as $evaluacion)
                                ...
                                @endphp
                                <td
                                    class="td-nota td-nota{{ $estudiant->id ?? '' }} {{ $class_pensum }} text-center table-{{ $nota ? 'light' : 'light' }}">
                        ...
                        @if (!empty($pevaluacion->evaluacions->first()))
                            <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                ...
                                <button type="button" class="btn-boletin btn btn-outline-primary"
                                    id="btn-boletin-{{ $estudiant->id }}">
                                    <i class="fa fa-save" aria-hidden="true"></i>
                                </button>

                                {{-- <a target="_blank" class="btn btn-outline-info" href="{{ $route=route('profesors.boletins.boletin.pdf',[$estudiant->id,$pevaluacion->lapso->id])}}" role="button">
...
{{-- <fieldset {{ ($pevaluacion->status_carga_notas) ? null:'disabled' }}> --}}
<fieldset {{ $disabled ?? null }}>
    <div class="btn-group btn-block">
        ...
        <a class="btn btn-light w-25" href="{{ url()->full() }}" role="button" title="Refrescar la página'">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $('.btn-boletin').click(function(e) {
            ...
            $('#btn-boletin-' + id).prop('disabled', true);
            ...
            $.post(url, data, function(result) {
                ...
                $('.btn-boletin').prop('disabled', false);
                Swal.fire({
                    title: 'ERROR',
                    icon: 'error'
                });
