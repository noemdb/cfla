{{-- @php $date= $coll_political->finicial @endphp --}}
{{-- @php $index= $coll_political->id @endphp --}}
@php ($chart = ['range'=>$date,'id_chart'=>'ingresoxdaydate_id_'.$index,'urlapi'=>route('administracion.ingresos.ingreso.day.date.chart'),'tipo'=>'line','limit'=>6 ])
@section('scripts')
    @parent
...
@component('administracion.elements.card.panel')
    ...
    @slot('header', 'Ingresos por Día ['.$date.']')
        ...
        @component('administracion.elements.canvas.chart')
            ...
            {{-- @slot('nav') --}}
                {{-- <nav class="nav nav-tabs ranges" id="nav-tab" role="tablist" data-canvas="{{ $chart['id_chart'] ?? ''}}" data-urlapi="{{ $chart['urlapi'] ?? ''}}" data-tipo="{{ $chart['tipo'] ?? ''}}" data-limit="{{ $chart['limit'] ?? ''}}"> --}}
                    {{-- <a data-range="{{$date ?? null}}" class="nav-item nav-link active" id="nav-3-tab" data-toggle="tab" href="#nav-3" role="tab" aria-controls="nav-3" aria-selected="false">3M</a> --}}
