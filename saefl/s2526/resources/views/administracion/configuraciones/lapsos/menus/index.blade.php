{{-- @component('elements.buttons.default')
    ...
    @slot('route', route('administracion.configuraciones.lapsos.create'))
    @slot('icon', $icon_menus['nuevo'])
@endcomponent --}}

@admin
@php $id_modal = 'modal_edit_create'; @endphp
<a title="Registrar Lapso" class="btn btn-primary" data-toggle="modal" data-target="#{{$id_modal ?? 'id_modal'}}" href="#"role="button">
    <i class="{{ $icon_menus['nuevo'] ?? ''}} fa-1x"></i>
</a>
@component('elements.widgets.modal')
    @slot('classH','secondary')
    @slot('id',$id_modal)
    @slot('title','Registrar Lapso')
    @slot('close',true)
    @slot('size','modal-md')
    @slot('body')
        @include('administracion.configuraciones.lapsos.partials.create')
    @endslot
@endcomponent
@endadmin
