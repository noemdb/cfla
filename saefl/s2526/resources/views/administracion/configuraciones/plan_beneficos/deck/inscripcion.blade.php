<div class="card card-primary mt-2">
    <div class="card-header">
        <h4 class=" pb-0 mb-0">
            Estudiantes con inscripciones registradas
            {{-- INI Menu rapido --}}
    ...
    <div class="card-body pt-1">
        <small class="font-weight-bold pb-1">
            Encontrados: <span class="">{{$inscripcions->count() ?? ''}}</span> ||
        ...
        <div class="row">
            @foreach($inscripcions as $inscripcion)
                {{-- {{$inscripcion->name ?? '123'}} --}}
                {{-- {{$inscripcion->lastname ?? '123'}} --}}
                {{-- @include('administracion.estudiant.deck') --}}
                <div class="col-sm-4 col-md-3 col-lg-2 pl-1 p-1 ">
                    @include('administracion.configuraciones.plan_beneficos.deck.card.inscripcion')
                </div>
            @endforeach
        </div>
        {{-- </ul> --}}
