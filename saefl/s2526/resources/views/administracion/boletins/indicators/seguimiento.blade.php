...
<div class="tab-content border border-top-0">
    @foreach($lapsos as $lapso)
        <div class="tab-pane fade {{($loop->iteration==$lapso_active->id) ? 'show active':''}}" id="tab{{$lapso->id}}">
            ...
            {{-- @include('administracion.boletins.indicators.partials.lapso.details') --}}
                    ...
                    @foreach($pestudios as $pestudio)
                        ...
                        <li class="nav-item">
                            <a class="nav-item nav-link {{($loop->iteration==1) ? 'active':''}}" href="#nested-{{$lapso->id}}-{{$pestudio->id}}" data-toggle="tab">
                                {{$pestudio->name}} 
                                <div class="text-muted d-block small" title="Cantidad promédio de evaluaciones por profesor">
                                    Cant. Prom. Notas.[{{round($ieePROM,2)}}]
                                </div>
                            </a>
                        </li>
                    @endforeach
                ...
                <div class="tab-content border border-top-0">
                    ...
                    <div class="tab-pane {{($loop->iteration==1) ? 'active':''}}" id="nested-{{$lapso->id}}-{{$pestudio->id}}">
                        <div class="p-2">
                            ...
                            @include('administracion.boletins.indicators.partials.profesors')
                        </div>
                    </div>
                    @endforeach
                </div>
