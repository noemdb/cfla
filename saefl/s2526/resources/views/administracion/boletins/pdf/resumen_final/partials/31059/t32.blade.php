...
<tr>
    ...
    @case(1) <td colspan="{{ $colspan_vi ?? ''}}" align="center" style="font-weight: bold">{{ $pestudio->name ?? '' }}
    </td> @break
    @case(2) <td colspan="{{ $colspan_vi ?? ''}}" align="center" style="font-weight: normal">CÓDIGO</td> @break
    @case(3) <td colspan="{{ $colspan_vi ?? ''}}" align="center" style="font-weight: bold">{{ $pestudio->code_oficial ??
        '' }}</td> @break
    @case(4) <td colspan="{{ $colspan_vi ?? ''}}" align="center" style="font-weight: normal">AÑO CURSADO</td> @break
    @case(5) <td colspan="{{ $colspan_vi ?? ''}}" align="center" style="font-weight: bold">{{ $grado->short_name ?? '' }}</td>
    @break
    @case(6) <td colspan="{{ $colspan_vi ?? ''}}" align="center" style="font-weight: normal">SECCIÓN</td> @break
    @case(7) <td colspan="{{ $colspan_vi ?? ''}}" align="center" style="font-weight: bold">{{ $seccion->name ?? '' }}
    </td> @break
    @case(8)
        <td colspan="{{ floor($colspan_vi/2) + 1 }}" align="center" style="font-weight: normal" class="text_small">N° DE ESTUDIANTES<br>POR SECCI&Oacute;N</td>
        <td colspan="{{ ceil($colspan_vi/2) - 1 }}" align="center" style="font-weight: normal; min-height: 10rem !important;" class="text_small">N° DE ESTUDIANTES<br> EN ESTA P&Aacute;GINA</td>
    @break
    @case(9)
    @php $rowspan = $pensums->count() - 8 @endphp
        <th rowspan="{{$rowspan}}" colspan="{{ floor($colspan_vi/2) + 1 }}" align="center" style="font-size: 0.7rem !important">{{ zfill($estudiants_full->count(),2) ?? '' }}</th>
        <th rowspan="{{$rowspan}}" colspan="{{ ceil($colspan_vi/2) - 1 }}" align="center" style="font-size: 0.7rem !important">{{ zfill($estudiants->count(),2) ?? '' }}</th>
    @break

    @endswitch

    {{-- @if ($loop->iteration==10) <td colspan="{{ $colspan_vi ?? ''}}" rowspan="{{ $rowspan ?? ''}}" align="center">*</td> @endif --}}
