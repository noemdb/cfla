...
@endphp
    ...
    <table width="100%" class="table table-striped table-hover table-sm p-1" id="table-data-default">
        ...
        <tbody id="tdatos">
            ...
            @foreach($registro_pago_combinados as $pago_combinado)
                ...
                <tr data-id="{{$pago_combinado->id}}" class="rounded-bottom {{ ($pago_combinado->status_irregular) ? 'table-danger': null}} ">
                    ...
                    <td> {{$pago_combinado->created_at->format('d-m-Y')}} @admin [{{$pago_combinado->id ?? ''}}] @endadmin </td>

                    <td>
                        <dl class="mb-0 small">
                            @foreach ($registropagos as $registropago)
                                <dd class="{{ ($registropago->status_unexpired) ? 'table-warning': null}}">
