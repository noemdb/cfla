<div class="card-body p-1">
    ...
    @php
        ...
        $activities = $indicador['activities'];
    @endphp
            ...
            <div class="row pb-2 text-center">
                ...
                <div class="col mx-2">
                    @component('academicos.elements.boxes.indicators')
                        ...
                        @slot('total',(!empty($indicador['porc_pevaluacions'])) ? $indicador['porc_pevaluacions']:0 )
                        @slot('unidad','%')
                        @slot('bars','true')
                    @endcomponent
                </div>
                <div class="col mx-2">
                    ...
                    @component('academicos.elements.boxes.indicators')
                        ...
                        @slot('total',(!empty($indicador['count_evaluacions'])) ? $indicador['count_evaluacions']:0 )
                    @endcomponent
                </div>
                <div class="col mx-2">
                    @php $promedio = (!empty($pensums->count())) ? round( ( $indicador['count_activities'] / $pensums->count()),0 ):null; @endphp
                    @component('academicos.elements.boxes.indicators')
                        @slot('title','NÚMERO DE ACTIVIDADES REGISTRADAS')
                        @slot('subtitle', 'En promedio '.$promedio.' actividades por asignatura')
                        @slot('class','secondary')
                        @slot('icon',$icon_menus['evaluacion'])
                        @slot('total',(!empty($indicador['count_activities'])) ? $indicador['count_activities']:0 )
                    @endcomponent
                </div>
                {{-- <div class="col mx-2">
                    @php $promedio = (!empty($pensums->count())) ? round( ( $indicador['count_lessons'] / $pensums->count()),0 ):null; @endphp
                    @component('academicos.elements.boxes.indicators')
                        @slot('title','NÚMERO DE LECCIONES REGISTRADAS')
                        @slot('subtitle', 'En promedio '.$promedio.' evaluaciones por asignatura')
                        @slot('class','secondary')
                        @slot('icon',$icon_menus['evaluacion'])
                        @slot('total',(!empty($indicador['count_lessons'])) ? $indicador['count_lessons']:0 )
                    @endcomponent
                </div> --}}
                <div class="col mx-2">
                    @component('academicos.elements.boxes.indicators')
                        ...
                        @slot('total',(!empty($indicador['porc_notas_load'])) ? $indicador['porc_notas_load']:0 )
                        @slot('unidad','%')
                        @slot('bars','true')
                    @endcomponent
                </div>
