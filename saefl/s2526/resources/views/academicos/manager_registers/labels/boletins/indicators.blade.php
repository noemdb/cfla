<div class="card-body p-1">
    ...
    @php
        ...
        $pevaluacions_pensums = $pestudio->getPevaluacionsPensums($lapso->id);
        $pevaluacion_components = $pestudio->getPevaluacionComponents($lapso->id);
        $evaluacions = $pestudio->getEvaluacions($lapso->id);
        $pensums = $pestudio->getPensumsAllSeccion($lapso->id);
        $profesors = $pestudio->getProfesors($lapso->id);
        $boletins = $pestudio->getBoletins($lapso->id);
    @endphp
            ...
            <div class="row pb-2 text-center">
                ...
                <div class="col mx-2">
                    @component('academicos.elements.boxes.indicators')
                        ...
                        {{-- @slot('unidad',' pts') --}}
                        @slot('subtitle','Cantidad de A. de Formación según malla curricular' )
                    @endcomponent
                </div>
                ...
                <div class="col mx-2">
                    ...
                    @component('academicos.elements.boxes.indicators')
                        ...
                        @slot('bars',true )
                        @slot('unidad',' %')
                        @slot('subtitle','% de la carga académica' )
                    @endcomponent
                </div>
                <div class="col mx-2">
                ...
                <div class="col mx-2">
                    ...
                    @component('academicos.elements.boxes.indicators')
                        ...
                        {{-- @slot('bars',true ) --}}
                        {{-- @slot('unidad',' %') --}}
                        @slot('subtitle','Prom. de Evaluaciones por Plan de Evaluación.' )
                    @endcomponent
                </div>
                ...
                <div class="col mx-2">
                    @php
                        ...
                        $indice = ($goal_notas_load > 0) ? round(($real_notas_load/$goal_notas_load),2)  : 0 ;
                    @endphp
                    @component('academicos.elements.boxes.indicators')
                        ...
                        @slot('bars',true )
                        @slot('unidad',' %')
                        @slot('subtitle','% total de Carga de Notas' )
                    @endcomponent
                </div>
