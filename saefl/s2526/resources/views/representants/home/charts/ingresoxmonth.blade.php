@php ($chart = ['range'=>'6','id_chart'=>'ingresoxmonth_id_'.Auth::user()->id,'urlapi'=>route('representants.bancos.ingresoxmonth.chart'),'tipo'=>'line','limit'=>6 ])
@section('scripts')
    @parent
...
@component('administracion.elements.card.panel')
    ...
    @slot('header', 'Movimientos Bancarios Percibidos')
        ...
        @component('administracion.elements.canvas.chart')
            ...
            @slot('nav')
                <nav class="nav nav-tabs ranges" id="nav-tab" role="tablist" data-canvas="{{ $chart['id_chart'] ?? ''}}" data-urlapi="{{ $chart['urlapi'] ?? ''}}" data-tipo="{{ $chart['tipo'] ?? ''}}" data-limit="{{ $chart['limit'] ?? ''}}">
                    {{-- <a data-range="3" class="nav-item nav-link active disabled" id="nav-3-tab" data-toggle="tab" href="#nav-3" role="tab" aria-controls="nav-3" aria-selected="false">Últ. 3Meses</a> --}}
                    <a data-range="6" class="nav-item nav-link active disabled" id="nav-6-tab" data-toggle="tab" href="#nav-6" role="tab" aria-controls="nav-6" aria-selected="false">Últimos 6 Meses</a>
