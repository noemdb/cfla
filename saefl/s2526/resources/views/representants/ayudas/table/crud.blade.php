                ...
                <tr data-id="{{$inscripcion->id ?? ''}}">
                    ...
                    <td class="{{ $class_action ?? '' }}" id="btn-action-{{ $estudiant->id }}">
                        ...
                        <div class="btn-group btn-group-sm">
                            <a title="Contancia de Inscripción" class="btn btn-primary" href="{{route('representants.constancia.inscripcions.pdf',$estudiant->id)}}" target="_blank" role="button">
                                <i class="{{ $icon_menus['pdf'] ?? ''}} fa-1x text-light"></i>
                            </a>
                            <a title="Contancia de Estudio" class="btn btn-info" href="{{route('representants.inscripcions.constancia.estudio.pdf',$estudiant->id)}}" target="_blank" role="button">
                                <i class="{{ $icon_menus['pdf'] ?? ''}} fa-1x text-light"></i>
                            </a>
