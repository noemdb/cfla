@php ($chart = ['range'=>null,'id_chart'=>'actividades_id_'.Auth::user()->id,'urlapi'=>route('representants.charts.control.evaluacions.actividades'),'tipo'=>'line','limit'=>10,'legend'=>true,'y_axes'=>'Cantidad de Evaluaciones','x_axes'=>'Días'])
@section('scripts')
    @parent
        ...
        @component('administracion.elements.canvas.chart')
            ...
            @slot('nav')
                <nav class="nav nav-tabs ranges" id="nav-tab" role="tablist" data-canvas="{{ $chart['id_chart'] ?? ''}}" data-urlapi="{{ $chart['urlapi'] ?? ''}}" data-tipo="{{ $chart['tipo'] ?? ''}}" data-limit="{{ $chart['limit'] ?? ''}}">
                    <a data-range="" class="nav-item nav-link active" id="nav-1-tab" data-toggle="tab" href="#nav-1" role="tab" aria-controls="nav-3" aria-selected="false"> <span class="small font-weight-bold">General</span></a>
                    <a data-range="1" class="nav-item nav-link " id="nav-1-tab" data-toggle="tab" href="#nav-1" role="tab" aria-controls="nav-3" aria-selected="false"> <span class="small font-weight-bold">1er Lapso</span></a>
