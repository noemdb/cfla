    ...
    <table width="100%" class="table table-striped table-hover table-sm small p-1" id="table-data-default">
        ...
        <tbody id="tdatos">
            ...
            @foreach($estudiants as $estudiant)
                ...
                @php $ammount_expire_bill = $estudiant->ammount_expire_bill; @endphp
                ...
                <tr data-id="{{$estudiant->id ?? ''}}" data-ci_estudiant="{{$estudiant->ci_estudiant ?? ''}}">
                    ...
                    <td id="td-estudiant-{{ $estudiant->id }}" class="{{ $class_estudiant  ?? ''}}">
                        <a class="btn-link" href="{{ route('representants.estudiants.index',['search'=>$estudiant->ci_estudiant]) }}">
                    ...
                    <td class="{{ $class_action ?? '' }}" id="btn-action-{{ $estudiant->id }}">
                        <div class="btn-group btn-group-sm">
                            ...
                            <a title="Resumen" class="btn-card btn btn-info" href="#">
...
@section('scripts')
    ...
    <script>
        $('.btn-card').click(function (e) {
            ...
            var ajaxurl = '{{route("representants.ajax.fill.modal.estudiant_card", "_id_")}}';
        ...
        $('.btn-retiro').click(function (e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    var ajaxurl = '{{route("representants.ajax.api.preinscripcions.retiro", "_id_")}}'; ajaxurl = ajaxurl.replace('_id_', id);
                    $.ajax({
                        url: ajaxurl,
                        type: "GET",
                        success: function(data){
