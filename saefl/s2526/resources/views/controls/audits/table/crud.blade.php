        ...
        <tr data-id="{{$evaluacion->id}}" data-evaluacion="{{$evaluacion->id ?? ''}}" class="table-{{(empty($evaluacion->administrativa->id)) ? 'default':'success'}}">
            ...
            <td class="{{ $class_action ?? '' }}" id="btn-action-{{ $evaluacion->id }}">
                <div class="btn-group">
                    ...
                    <a title="Editar" class="btn btn-warning {{ $disabled }}"  href="{{route('controls.evaluacions.edit',$evaluacion->id)}}" role="button">
                        <i class="{{ $icon_menus['editar'] ?? ''}} fa-1x"></i>
                    </a>

                    <a title="Registro de Notas" class="btn btn-primary" href="{{route('controls.boletins.carga',['pevaluacion_id'=>$pevaluacion->id])}}" role="button">
...
{{-- </div> --}}
{!! Form::open(['route' => ['controls.evaluacions.destroy',':EVALUACION_ID'], 'method' => 'DELETE', 'id'=>'form-destroy', 'role'=>'form']) !!}
{!! Form::close() !!}
@section('scripts')
    @parent
