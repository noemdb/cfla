            ...
            class="table-{{(empty($pevaluacion->administrativa->id)) ? 'default':'success'}}">
                ...
                <td class="{{ $class_action ?? '' }}" id="btn-action-{{ $pevaluacion->id }}">
                    <div class="btn-group">
                        ...
                        <a title="Registro de Notas" class="btn {{$class_btn ?? ''}} {{$disabled ?? ''}}" href="{{route('controls.boletins.carga',['pevaluacion_id'=>$pevaluacion->id])}}" role="button">
                            <i class="{{ $icon_menus['notas'] ?? ''}} fa-1x"></i>
                        </a>
                        <a title="Acta de Notas por Profesor" target="_blank" class="btn btn-info" href="{{route('controls.boletins.sabana.pdf',['pevaluacion_id'=>$pevaluacion->id])}}" role="button">
