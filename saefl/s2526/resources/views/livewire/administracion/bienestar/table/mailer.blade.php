@php
    ...
    // $class['grado_seccion']="d-none d-sm-table-cell";
    ...
    // $class['action']= ($modeIndex) ? "d-none d-md-table-cell" : "d-none";
...
<table width="100%" class="table table-striped table-hover table-sm small p-1" id="{{$table_id}}">
    <thead>
        <tr>
            ...
            {{-- <th class="{{ $class['status_sender'] ?? ''}}">{{$list_comment['status_sender'] ?? ''}}</th> --}}
    ...
    <tbody id="tdatos-{{$table_id}}">
        ...
        @forelse($mailers as $mailer)
            ...
            <tr data-id="{{$mailer->id}}" class="{{($mailer->id == $mailer_id) ? 'bg-secondary font-weight-bold text-light' : null}}">
                ...
                {{-- <td class="{{ $class['status_sender'] ?? ''}}">{{$mailer->status_sender ?? ''}}</td> --}}
                <td class="{{ $class['action'] ?? ''}}">
                    {{-- @includeWhen(true,'livewire.academico.mailer.table.partials.btnModeIndex',['key'=>'table']) --}}
