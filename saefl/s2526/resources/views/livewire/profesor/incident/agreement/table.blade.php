                    ...
                    <td id="td-count" class="{{ $class_estudiant }} {{ $displaModeIndex }}">
                        @forelse ($incident_agreements as $incident_agreement)
                            <div title="{{$incident_agreement->description ?? null}}">
                                <span class="text-dark"> {{$loop->iteration}}. {{ Str::limit($incident_agreement->description,32,'...')}}</span>
                            </div>
                        @empty
                            ...
                            <div class="d-flex justify-content-start">
                                ...
                                <div class="btn-group btn-group-sm px-1" >
                                    @php
                                        ...
                                        // $disabled = ( ! $status_notify && (! $incident->status_notify_agreement || ! $incident->status_close)) ? true : false;
                                        $disabled = ( $incident->status_notify_agreement || $incident->status_close ) ? true : false;
                                    @endphp
                                    <a title="Registrar acuerdo de la incidencia" class="btn btn-primary btn-sm {{ ($disabled) ? 'disabled':null }}" href="#" wire:click="create({{$incident->id ?? null}})" role="button">
                                ...
                                @if (! $incident_agreements->isEmpty())
                                    ...
                                    <div class="btn-group btn-group-sm ml-1" >
                                        ...
                                        @if ($incident->status_notify_agreement)
                                            ...
                                            <a title="Notificación enviada" class="btn btn-info btn-sm disabled" href="#" role="button">
                                                <i class="{{ $icon_menus['check'] ?? '' }} fa-1x"></i>
                                            </a>

                                        @else
                                            <a title="Enviar por correo una notificación del registro del acuerdo de la incidencia" class="btn btn-success btn-sm" href="#" wire:click="sendMail({{$incident->id}})" role="button">
                                    ...
                                    <div class="dropdown dropleft">
                                        <button class="btn btn-secondary dropdown-toggle btn-sm ml-1" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            <i class="{{ $icon_menus['incident_setting'] ?? '' }} fa-1x"></i>
                                        </button>
                                        <div class="dropdown-menu px-1 mx-1" aria-labelledby="dropdownMenuButton"  wire:key="incident_btn_{{$incident->id}}">
                                            @foreach ($incident_agreements as $incident_agreement)

                                                <div class="dropdown-item" title="{{$incident_agreement->description ?? null}}"  wire:key="incident_agreement_btn_{{$incident_agreement->id}}">
                                                    <div class="d-flex justify-content-between">
                                                        <div title="{{$incident_agreement->description ?? null}}">
                                                            <span class="small text-dark"> {{$loop->iteration}}. {{ Str::limit($incident_agreement->description,32,'...')}}</span>
                                                        </div>
                                                        <div class="btn-group btn-group-sm  px-1 mx-1" >

                                                            <a title="Editar datos del acuerdo de la incidencia" class="btn btn-warning btn-sm {{ ($incident->status_notify_agreement) ? 'disabled' : null}}" href="#" wire:click="edit({{$incident_agreement->id}})" role="button">
                                                                <i class="{{ $icon_menus['editar'] ?? '' }} fa-1x"></i>
                                                            </a>

                                                            <a title="Eliminar datos de la incidencia" class="btn btn-danger btn-sm {{ ($incident->status_notify_agreement) ? 'disabled' : null}}" href="#" wire:click="destroy({{$incident_agreement->id}})" role="button">
                                ...
                                <div class="btn-group btn-group-sm px-1" >
                                    ...
                                    {{-- @php $disabled = ($incident->status_notify_agreement || $incident->status_close || $incident_agreements->isEmpty()) ? true : false; @endphp --}}
