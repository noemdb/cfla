...
<head>
    ...
    <title>Planificación Quincenal - Formato 2</title>
    <style>
        body {
...
<body>
    ...
    @php
        $grado = $eiplanningbwk->grado;
        $seccion = $eiplanningbwk->seccion;
        $manager = $eiplanningbwk->manager;
        $peducativo = $eiplanningbwk->peducativo;
    @endphp

    <!-- Membrete -->
    @include('livewire.inicial.formats.eiplanningbwk.membrete')
    ...
    <table class="cabecera" class="format" width="100%" cellpadding="0" cellspacing="0" style="font-size:0.8rem;margin-bottom:0.2rem; padding-bottom:0.2rem;">
        <tr class="">
            ...
            <td>
                ...
                <p><span>Fecha de Culminación </span>: {{$eiplanningbwk->ffinal}}</p>
                <p><span>Tiempo de ejecución Semana </span>: {{$eiplanningbwk->tiempo_ejecucion}} </p>
            </td>
        ...
        <tr>
            <td colspan="2">
                <p><span>Diagnóstico </span>: {!! as_replace($eiplanningbwk->diagnostico) !!}.</p>
            </td>
        </tr>
    </table>
    ...
    <table class="format" width="100%" cellpadding="0" cellspacing="0" style="font-size:0.8rem;margin-bottom:0.2rem; padding-bottom:0.2rem;">
        ...
        <tbody>
            @php
                $eiplanningbwsummaries = $eiplanningbwk->getOrderedSummaries(); //dd($eiplanningbwsummaries);
            @endphp
            @forelse ($eiplanningbwsummaries as $item)
            @php
                $pevaluacion = $item->pevaluacion; //dd($pevaluacion);
            ...
            <tr>
                <td colspan="7">Observación <small>[Coord. Evaluación]</small>: {{$eiplanningbwk->observacion}}</td>
            </tr>
            <tr>
                <td colspan="7">Firma del Docente:</td>
            </tr>
    ...
    <table class="format" style="font-size:0.8rem;margin-bottom:0.2rem; padding-bottom:0.2rem;">
        <thead>
            <tr>
                ...
                @foreach($eiplanningbwk->week_days as $day_key => $day_name)
                    <th>{{ $day_name }}</th>
                @endforeach
            </tr>
        </thead>
        <tbody>
            @foreach($eiplanningbwk->list_moment as $momento_key => $momento_name)
            <tr>
                ...
                @foreach($eiplanningbwk->week_days as $day_key => $day_name)
                    <td>
                        @php
                            $estrategia = $eiplanningbwk->getStrategyByMomentAndDay($momento_key, $day_key);
                        @endphp
            ...
            <tr>
                <td colspan="6">Observación <small>[Coord. Evaluación]</small>: {{$eiplanningbwk->observacion}}</td>
            </tr>
            <tr>
