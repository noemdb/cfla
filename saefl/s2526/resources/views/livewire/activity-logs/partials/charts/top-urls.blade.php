<div class="card">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="card-title mb-0">Top 6 URLs más Accedidas</h5>
        </div>
...
function initTopUrlsChart(topUrlsData) {
    const chartElement = document.querySelector("#topUrlsChart");
    if (!chartElement) return;

    if (!topUrlsData || topUrlsData.length === 0) {
        chartElement.innerHTML = '<div class="text-center p-4"><p class="text-muted">No hay datos disponibles</p></div>';
        return;
    }

    const urlLabels = topUrlsData.map(item => item.url);
    const urlValues = topUrlsData.map(item => item.count);
    ...
    if (topUrlsChart) {
        topUrlsChart.destroy();
    }

    chartElement.innerHTML = '';
    topUrlsChart = new ApexCharts(chartElement, options);
    topUrlsChart.render();
}

// Inicializar el chart cuando el componente se carga
document.addEventListener('DOMContentLoaded', function() {
    initTopUrlsChart(@json($stats['topUrls']));
});

// Escuchar el evento de actualización de datos
document.addEventListener('livewire:load', function() {
    Livewire.on('topUrlsDataUpdated', (data) => {
        initTopUrlsChart(data);
    });
});
</script>
@endsection
