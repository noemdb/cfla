    ...
    <table width="100%" class="table table-striped table-hover table-sm small p-1" id="table-data-default">
        <thead>
            <tr>
                ...
                <th class="{{ $class_estudiant }} {{ $displaModeIndex }}">N. Acuerdos registrados</th>
        ...
        <tbody id="tdatos">
            ...
            @foreach ($estudiants as $estudiant)
                ...
                <tr data-estudiant_id="{{ $estudiant->id ?? '' }}" data-id="{{ $estudiant->id ?? '' }}">
                    ...
                    <td class="{{ $class_estudiant ?? '' }} {{ $displaModeIndex }}">
                        {{ $incident_agreements->count() ?? null }}
                    </td>

                    <td class="{{ $class_estudiant ?? '' }} {{ $displaModeIndex }}">
                        <span class=" font-weight-bold text-{{ $ammount <= 0 ? 'success' : 'danger' }}">
                            {{ $ammount <= 0 ? 'SOLVENTE' : 'MOROSO' }}</span>
                    </td>

                    <td class="{{ $class_action ?? '' }}">
                        {{-- <div class="btn-group btn-group-sm" wire:key="estudiant-table-action-group-btn-{{$estudiant->id}}" id="estudiant-table-action-group-btn-{{$estudiant->id}}"> --}}
                        {{-- <div wire:key="estudiant-table-action-group-btn-{{$estudiant->id}}"> --}}
                        <div class="btn-group btn-group-sm">
                            <a title="Generar ficha" target="_BLANK"
                                class="btn btn-dark bnt-sm {{ $incidents->isEmpty() ? 'disabled' : null }}"
                                href="{{ route('bienestars.estudiants.pdf.ficha', $estudiant->id) }}" role="button">
                                <i class="{{ $icon_menus['pdf'] ?? '' }} fa-1x"></i>
                            </a>
                            <a title="Mostrar resumen" class="btn btn-info bnt-sm"
                                wire:click="viewInfo({{ $estudiant->id }})" href="#" role="button">
                                <i class="{{ $icon_menus['info'] ?? '' }} fa-1x"></i>
                            </a>

                            @if ($incidents->count() > 0)
                                <a title="Línea de tiempo" class="btn btn-light btn-sm mr-1" href="#"
                                    wire:click="tline({{ $estudiant->id }})" role="button">
                                    <i class="{{ $icon_menus['tline'] ?? '' }} fa-1x text-info"></i>
                                </a>
                            @endif
                        </div>
                        {{-- </div> --}}
