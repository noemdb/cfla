...
/**
 * "Guardar sección" debe permitir el docente compartido (ambas lessons con
 * allow_shared_teacher) y persistir el flag en el slot.
 */
class TimetablePersistSectionSharedTeacherTest extends TestCase
{
    ...
    public function test_persist_allows_shared_teacher_pair(): void
    {
        ...
        $profesor = Profesor::create([
            ...
            'ci_profesor' => '1701', 'status_active' => 'true',
        ]);
        ...
        // Slot preservado de la sección B en el período destino (docente compartido).
        ...
        // La lección A se guardó en el mismo período (compartido).
        $slotA = TimetableSlot::query()->where('lesson_id', $lessonA->id)->first();
        $this->assertNotNull($slotA);
        $this->assertSame($period->id, (int) $slotA->period_id);
        $this->assertTrue((bool) $slotA->allow_shared_teacher, 'el flag se persiste en el slot');
    }
}
