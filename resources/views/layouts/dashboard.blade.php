    ...
    <header class="bg-gray-900/50 backdrop-blur-md border-b border-white/5 sticky top-0 z-50">
        <div class="container mx-auto px-4 py-3">
            <div class="flex items-center justify-between">
                ...
                <!-- Navigation -->
                <nav class="hidden lg:flex items-center space-x-1 mr-auto">
                    {{-- Dropdown: Admin --}}
                    ...
                    @elseif(Auth::user()->is_planner)
                        {{-- Dropdown: Planificación (mega-dropdown con cascada, solo planner) --}}
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" @click.outside="open = false"
                                class="inline-flex items-center gap-1.5 text-sm font-medium rounded-lg px-3 py-1.5 transition-all duration-200 {{ request()->routeIs('app.planning.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-400 hover:text-emerald-300 hover:bg-white/5' }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Navbar Info (inyectado por vistas hijas) -->
                @yield('navbar-info')

                <!-- User Profile -->
                <div class="flex items-center space-x-4 px-2 py-1 bg-gray-900/30 backdrop-blur-md rounded-lg mx-2">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="p-2 text-gray-400 hover:text-white bg-white/5 hover:bg-emerald-500/20 rounded-lg border border-white/5 transition-all duration-300 group">
