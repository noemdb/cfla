...
<head>
    ...
    <title>Horarios del grado</title>
    <style>
...
<body>
    <div class="page-header">
        ...
        <h2>{{ ($isPublishedSchedule ?? false) ? 'HORARIOS DE CLASES' : 'VISTA PREVIA — HORARIOS DE CLASES' }} — {{ $grado->name }}</h2>
        <div class="subhead">{{ $calendar->name }} · {{ $fecha }}</div>
    </div>

    @forelse ($sectionSchedules as $schedule)
        <div class="section {{ $loop->first ? '' : 'new-page' }}">
            <div class="section-title">Sección {{ $schedule['section']->name }}</div>
    ...
    @empty
        <div class="subhead">No hay horarios asignados para las secciones de este grado.</div>
    @endforelse
