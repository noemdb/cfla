...
<head>
    ...
    <title>Horarios de todos los P.Estudios</title>
    <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .pestudio-title{background:#0d9488;color:#fff;font-size:8pt;font-weight:800;padding:4px 6px;margin-top:9px;}
        .pestudio-title.new-page{page-break-before:always;}
        .pestudio-title .calendar-label{float:right;font-weight:600;color:#d1fae5;}
...
<body>
    <div class="page-header">
        ...
        <h2>HORARIOS DE CLASES — TODOS LOS P.ESTUDIOS</h2>
        <div class="subhead">{{ $lapso->name }} · {{ $fecha }}</div>
    </div>

    @forelse ($pestudiosData as $pestudioData)
        @php
            $calendar = $pestudioData['calendar'];
            $pestudio = $pestudioData['pestudio'];
        @endphp
        <div class="pestudio-title {{ $loop->first ? '' : 'new-page' }}">
            P.Estudio: {{ $pestudio->name }}
            <span class="calendar-label">{{ $calendar->name }}</span>
        </div>
        ...
        @endforeach
    @empty
        <div class="subhead">No hay horarios disponibles para los P.Estudios del lapso vigente.</div>
    @endforelse
