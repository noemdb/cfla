            ...
            <div class="hidden sm:flex sm:items-center sm:justify-between w-full">
                ...
                <div>
                    <span class="relative z-0 inline-flex items-center gap-1">
                        ...
                        @else
                            <button type="button" wire:click="previousPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.after" rel="prev" aria-label="Anterior"
                        ...
                        {{-- Next Page Link --}}
                        @if ($paginator->hasMorePages())
                            <button type="button" wire:click="nextPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.after" rel="next" aria-label="Siguiente"
                                class="relative inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-bold text-gray-400 bg-gray-800/30 border border-white/5 hover:bg-gray-700 hover:text-white hover:border-white/10 transition-all duration-200">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
