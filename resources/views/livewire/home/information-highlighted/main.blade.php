<div class="container-fluid mx-auto px-2 sm:px-2 lg:px-2 py-2">
    <div
        ...
        <div class="relative rounded-3xl overflow-hidden shadow-2xl h-[350px] sm:h-[450px] md:h-[550px] group border border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-gray-800/50"
            ...
            @if (count($images) > 0)
                <div class="swiper infoHighlightedSwiper w-full h-full">
                    ...
                    <!-- Pagination -->
                    @if (count($images) > 1)
                        <div class="swiper-pagination info-highlighted-pagination !bottom-4"></div>
                        <!-- Navigation Controls -->
                        <div
                            class="swiper-button-prev info-highlighted-prev !left-4 !w-10 !h-10 !bg-white/90 dark:bg-gray-800/90 backdrop-blur-md !rounded-full !text-emerald-600 after:!text-lg shadow-lg hover:!bg-emerald-500 hover:!text-white transition-all duration-300 opacity-0 group-hover:opacity-100 flex items-center justify-center">
                        </div>
                        <div
                            class="swiper-button-next info-highlighted-next !right-4 !w-10 !h-10 !bg-white/90 dark:bg-gray-800/90 backdrop-blur-md !rounded-full !text-emerald-600 after:!text-lg shadow-lg hover:!bg-emerald-500 hover:!text-white transition-all duration-300 opacity-0 group-hover:opacity-100 flex items-center justify-center">
                        </div>
                    @endif
                </div>
            @else
...
<style>
    .info-highlighted-pagination .swiper-pagination-bullet {
        width: 8px;
        height: 8px;
    ...
    .info-highlighted-pagination .swiper-pagination-bullet-active {
        width: 24px;
        border-radius: 4px;
        opacity: 1;
    }

    .info-highlighted-prev,
    .info-highlighted-next {
    ...
    function initInfoHighlightedSwiper() {
        ...
        if (swiperEl && !swiperEl.swiper && window.Swiper) {
            ...
            if (slideCount > 0) {
                ...
                new window.Swiper(".infoHighlightedSwiper", {
                    modules: [
                    ...
                    pagination: canLoop ? {
                        el: ".info-highlighted-pagination",
                        clickable: true,
                    } : false,
                    navigation: canLoop ? {
                        nextEl: ".info-highlighted-next",
                        prevEl: ".info-highlighted-prev",
                    } : false,
                });
            }
