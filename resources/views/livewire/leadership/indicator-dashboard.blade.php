        ...
        <div class="bg-gray-900/40 backdrop-blur-md border border-white/5 p-4 sm:p-5 rounded-lg transition-all duration-300 hover:border-emerald-500/30 hover:shadow-lg hover:shadow-emerald-500/5">
            ...
            <div class="mt-3 pt-3 border-t border-white/10 flex items-center justify-between gap-2 text-[10px] sm:text-[11px] text-gray-500 flex-wrap">
                <span class="flex items-center gap-1">
                    ...
                    Estudiantes: <strong class="text-emerald-400 font-bold">{{ number_format($diagStudents) }}</strong>
                </span>
                <span class="flex items-center gap-1">
    ...
    <div class="bg-gray-900/40 backdrop-blur-md border border-white/5 rounded-lg overflow-hidden" x-data="{ activeTab: {{ $activeTab }} }">
        ...
        <div class="p-2 sm:p-4 lg:p-6">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 bg-amber-100 dark:bg-amber-500/20 rounded-lg flex items-center justify-center">
                                <svg class="w-4 h-4 text-amber-600 dark:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                            </div>
                            <div>
                            ...
                            <nav class="flex overflow-x-auto gap-0.5 snap-x snap-mandatory -mb-px">
                                @foreach($peducativoMainIndicators as $item)
                                    <button @click="activePeducativo = {{ $item->peducativo->id }}"
                                        :class="activePeducativo === {{ $item->peducativo->id }} ? 'text-amber-400 border-amber-500 bg-amber-500/5' : 'text-gray-500 border-transparent hover:text-gray-400'"
                                    ...
                                    @foreach(['7d' => '7 días', '30d' => '30 días', '3m' => '3 meses', 'all' => 'Todo'] as $val => $label)
                                        <button wire:click="$set('chartActivitiesRange', '{{ $val }}')"
                                            class="px-2 sm:px-3 py-1.5 min-h-[36px] text-[10px] font-bold uppercase tracking-wider rounded-md transition-all duration-200 whitespace-nowrap
                                                   {{ $chartActivitiesRange === $val ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/5' }}">
                                            {{ $label }}
                                        </button>
                                    @endforeach
                                </div>
                                ...
                                <div class="flex items-center gap-1 bg-gray-100 dark:bg-white/5 rounded-lg p-0.5">
                                    @foreach(['7d' => '7 días', '30d' => '30 días', '3m' => '3 meses', 'all' => 'Todo'] as $val => $label)
                                        <button wire:click="$set('chartScheduledRange', '{{ $val }}')"
                                            class="px-2 sm:px-3 py-1.5 min-h-[36px] text-[10px] font-bold uppercase tracking-wider rounded-md transition-all duration-200 whitespace-nowrap
                                                   {{ $chartScheduledRange === $val ? 'bg-violet-500/20 text-violet-600 dark:text-violet-400 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/5' }}">
                                            {{ $label }}
                                        </button>
                                    @endforeach
                                </div>
            ...
            <div x-show="activeTab === 4" x-cloak
                        ...
                        <div x-show="activePeducativo === {{ $peducativo->id }}" x-cloak>
                            ...
                            @else
                                <div class="bg-white/5 rounded-lg p-4 sm:p-6 text-center">
                                    ...
                                    <p class="text-gray-600 text-xs">No hay lecciones registradas para este período en {{ $peducativo->name }} bajo tu supervisión.</p>
                                </div>
                            @endif
                        </div>
                    @endforeach
                @else
                    <div class="bg-white/5 rounded-lg p-4 sm:p-6 text-center">
                        <p class="text-gray-500 text-sm">No hay datos de lecciones para el período seleccionado en tus áreas asignadas.</p>
                    </div>
                @endif
            </div>
