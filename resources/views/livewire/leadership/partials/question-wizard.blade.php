                ...
                @elseif($wizardStep === 2)
                    ...
                    @if($tipo_pregunta === 'multiple')
                        <div>
                            ...
                            @if(count($options) < 6)
                                <button wire:click="addOption" class="mt-2 inline-flex items-center gap-1 text-[11px] text-gray-500 hover:text-purple-400 transition-colors">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                                    Añadir opción
                                </button>
                            @endif
                            @error('options') <p class="text-[10px] text-red-400 mt-1">{{ $message }}</p> @enderror
                            @error('correct_option_index') <p class="text-[10px] text-red-400 mt-1">{{ $message }}</p> @enderror
                        </div>
                    ...
                    @elseif($tipo_pregunta === 'scale')
                        <div>
                            ...
                            @error('max_value') <p class="text-[10px] text-red-400 mt-1">{{ $message }}</p> @enderror
                        </div>
                    @endif
                ...
                @elseif($wizardStep === 3)
                    ...
                    @if($pregunta)
                        <div class="bg-gray-800/30 border border-white/5 rounded-lg p-4">
                            ...
                            @elseif($tipo_pregunta === 'open')
                                <textarea disabled rows="2" class="w-full bg-gray-800/30 border border-white/5 rounded-lg px-3 py-2 text-xs text-gray-500" placeholder="Respuesta libre..."></textarea>
                            @elseif($tipo_pregunta === 'scale')
                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] text-gray-500">{{ $min_value ?? 1 }}</span>
                                    <div class="flex-1 h-2 bg-gray-700/50 rounded-full"><div class="w-0 h-2 bg-purple-500 rounded-full"></div></div>
