<div class="fade-in space-y-6">
    {{-- Header --}}
    <div>
        <h1 class="text-lg font-extrabold text-gray-900 dark:text-white mb-2">Lista de Profesores</h1>
        <p class="text-emerald-600 dark:text-emerald-400 font-medium text-sm">
            {{ count($profesores) }} profesor(es) registrados
        </p>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div x-data="{ open: true }"
            ...
            <button @click="open = !open" type="button"
                class="w-full flex items-center justify-center lg:justify-end px-1 py-1 text-gray-500 hover:text-emerald-400 transition-colors mb-1">
            ...
            <div x-show="open" x-cloak>
                <div class="space-y-3">
                    {{-- Header --}}
                    <div class="text-[10px] font-bold uppercase tracking-widest text-emerald-400/60 px-1 pb-2 border-b border-white/5">
                        Profesores
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <input wire:model.live.debounce.300ms="searchProfesor" type="text" placeholder="Buscar profesor..."
                            class="w-full pl-9 pr-8 py-2 text-sm bg-white/5 border border-white/10 rounded-lg text-gray-300 placeholder-gray-600 outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500/40 transition-all">
                        @if($searchProfesor)
                    ...
                    <div class="space-y-1 max-h-[48vh] overflow-y-auto">
                        @forelse($filteredProfesores as $prof)
                            <button wire:key="prof-{{ $prof->id }}" wire:click="selectProfesor({{ $prof->id }})"
                                class="w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center gap-3 {{ $selectedProfesorId == $prof->id ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-gray-300 hover:text-emerald-300 hover:bg-white/5' }}">
            ...
            @if($profesor && $kpi)
                ...
                <div class="rounded-2xl border border-white/5 bg-gray-900 p-5">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex items-start gap-4 min-w-0">
                            {{-- Avatar placeholder --}}
                            <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 border border-emerald-500/20 flex items-center justify-center shrink-0">
                ...
                <div class="rounded-2xl border border-white/5 bg-gray-900 p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <div class="rounded-xl bg-emerald-500/5 border border-emerald-500/10 p-3.5 text-center space-y-1">
                            <div class="text-2xl font-black text-emerald-400">{{ $resumenActividades['pending'] }}</div>
                            <div class="text-[10px] font-medium text-emerald-400/60">Pendientes</div>
                        </div>
                    </div>
                ...
                <div class="rounded-2xl border border-white/5 bg-gray-900 p-5 space-y-4">
                    ...
                    @if($resumenLecciones->isNotEmpty())
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            ...
                            {{-- Scheduled --}}
                            <div class="rounded-xl bg-emerald-500/5 border border-emerald-500/10 p-3.5 text-center space-y-1">
                                <div class="text-xl font-black text-emerald-400">{{ $resumenLecciones['SCHEDULED'] ?? 0 }}</div>
                                <div class="text-[10px] font-medium text-emerald-400/60">Programadas</div>
                            </div>
                            {{-- Published --}}
                            <div class="rounded-xl bg-emerald-500/5 border border-emerald-500/10 p-3.5 text-center space-y-1">
                        ...
                        <div class="flex items-center gap-1 bg-gray-100 dark:bg-white/5 rounded-lg p-0.5">
                            @foreach(['7d' => '7 días', '30d' => '30 días', '3m' => '3 meses', 'all' => 'Todo'] as $val => $label)
                                <button wire:click="$set('activityRange', '{{ $val }}')"
                                    ...
                                    {{ $activityRange === $val ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/5' }}">
                                    {{ $label }}
                                </button>
                            @endforeach
                        </div>
