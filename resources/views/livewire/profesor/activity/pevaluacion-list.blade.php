        ...
        <div class="mb-3">
            <form wire:submit.prevent="$refresh" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2">
                ...
                <div>
                    ...
                    <select wire:model.live="grado_id" @if(!$pestudio_id) disabled @endif
                        class="w-full bg-gray-800/50 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-gray-300 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                        <option value="">{{ $pestudio_id ? 'Seleccione' : 'Seleccione un plan primero' }}</option>
                ...
                <div>
                    ...
                    <select wire:model.live="seccion_id" @if(!$grado_id) disabled @endif
                        class="w-full bg-gray-800/50 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-gray-300 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                        <option value="">{{ $grado_id ? 'Seleccione' : 'Seleccione un grado primero' }}</option>
