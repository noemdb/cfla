<div class="pb-2">
    ...
    <x-inputs.maskable label="{{$label}}" mask="################" placeholder="{{$label}}" wire:model.live="{{$model}}"
        right-icon="calculator" corner-hint="Sólo números" />
</div>

<div class="pb-2">
    @php $name = 'banco_id_1' ; $model = 'payment.'.$name; $label=$list_comment[$name] @endphp
    <x-select label="{{$label}}" placeholder="Seleccionar" :options="$list_bank" wire:model.live="{{$model}}"
        option-key-value />
</div>

<div class="pb-2">
    @php $name = 'banco_emisor_1' ; $model = 'payment.'.$name; $label=$list_comment[$name] @endphp
    <x-select label="{{$label}}" placeholder="Seleccionar" :options="$banco_emisor_list" wire:model.live="{{$model}}"
        option-key-value />
</div>
...
<div class="pb-2">
    ...
    <x-select label="{{$label}}" placeholder="Seleccionar" :options="$method_pay_list" wire:model.live="{{$model}}"
        option-key-value />
</div>
