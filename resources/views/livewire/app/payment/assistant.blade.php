    ...
    <div class="my-2 p-4 bg-emerald-900/30 border border-emerald-500/30 rounded-lg flex justify-between items-center">
        <div class="text-emerald-400 font-bold">
            <span class="text-xl font-bold">Representante encontrado.</span> <br> <span
                class="text-sm text-gray-300 text-left">Nombre: {{ $representant->name ?? null }}, CI:
                {{ $representant->ci_representant ?? null }}</span>
        </div>
        <div><x-badge.circle icon="check" positive class="w-10 h-10" /></div>
    </div>

    @php $width = ($limit>0) ? round((100 * $step / $limit)) : 0; @endphp
    <div class="mb-6 h-1 w-full bg-gray-700 rounded-full">
        <div class="h-1 bg-emerald-500 rounded-full transition-all duration-500 ease-in-out"
