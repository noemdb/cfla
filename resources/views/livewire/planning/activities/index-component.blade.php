<div class="fade-in" x-data="{
    ...
    @if($profesor_id && $pevaluacions->total() > 0)
        ...
        <div class="bg-{{ $c }}-500/10 border border-{{ $c }}-500/20 p-5 rounded-2xl mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-{{ $c }}-500/20 rounded-2xl flex items-center justify-center">
                    ...
                    <div>
                        <p class="text-{{ $c }}-300 text-sm font-bold uppercase tracking-wider">Indicador — Actividades sobre el Promedio</p>
    ...
    <div class="bg-gray-900/40 backdrop-blur-md border border-white/5 rounded-2xl overflow-hidden">
        ...
        <div class="space-y-6 p-6">
            ...
            @empty
                <div class="bg-gray-900/20 border border-white/5 rounded-2xl py-16 text-center">
        ...
        <div class="space-y-4">
            @if($activity)
                <div class="bg-white/5 p-4 rounded-xl border border-white/5">
                    ...
                    <p class="text-sm text-white font-medium">{{ $activity->topic }}</p>
                    <p class="text-xs text-gray-300 mt-1 line-clamp-2">{{ $activity->teaching }}</p>
                </div>
            @endif

            <div>
                <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">Estado de Aprobación</label>
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2 cursor-pointer group">
                        <input type="radio" wire:model="status" value="1"
                            class="w-4 h-4 text-emerald-500 bg-white/5 border-white/10 focus:ring-emerald-500/50 focus:ring-2">
                        <span class="text-sm text-gray-300 group-hover:text-white transition-colors">Aprobado</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer group">
                        <input type="radio" wire:model="status" value="0"
                            class="w-4 h-4 text-amber-500 bg-white/5 border-white/10 focus:ring-amber-500/50 focus:ring-2">
                        <span class="text-sm text-gray-300 group-hover:text-white transition-colors">En revisión</span>
                    </label>
                </div>
            </div>
                ...
                <div class="bg-white/5 p-4 rounded-xl border border-white/5">
                    <div class="flex items-center justify-between mb-1.5">
                        ...
                        @if($previewActivity->hasTeachingStructure())
                            <button @click="showTeaching = !showTeaching"
                                class="text-[10px] font-bold uppercase tracking-wider text-sky-400 hover:text-sky-300 transition-colors">
                    ...
                    @if(!empty($sections))
                        <div x-show="showTeaching" x-cloak x-transition:enter.duration.200ms>
                            <div class="space-y-3">
                                <div class="bg-cyan-500/5 border border-cyan-500/10 rounded-lg p-3">
                                ...
                                <div class="bg-emerald-500/5 border border-emerald-500/10 rounded-lg p-3">
                    ...
                    <div x-show="!showTeaching">
                        <p class="text-sm text-gray-200 whitespace-pre-wrap">{{ $previewActivity->teaching ?? '—' }}</p>
                    </div>
                </div>
                            ...
                            <ul class="space-y-1.5">
                                @foreach($previewActivity->achievements as $ach)
                                    <li class="flex items-center gap-2 text-sm text-gray-200">
                                        ...
                                        <span>{{ $ach->name }}</span>
                                        @if($ach->weighting)
                                            <span class="text-[10px] font-bold text-emerald-400 bg-emerald-500/10 px-1.5 py-0.5 rounded-md">[{{ $ach->weighting }}]</span>
                                        @endif
                                    </li>
                                @endforeach
                            </ul>
                        @else
                            <p class="text-sm text-gray-500 italic">Sin indicadores de logro</p>
                        @endif
                    </div>
                ...
                @if($previewActivity->comments)
                    <div class="bg-amber-500/5 border border-amber-500/10 p-4 rounded-xl">
                        <div class="flex items-center gap-2 mb-1.5">
                            ...
                            <span class="text-[10px] font-bold uppercase tracking-widest text-amber-400">Comentario del Jefe de Área</span>
                        </div>
                        <p class="text-sm text-amber-200 italic">"{{ $previewActivity->comments }}"</p>
                    </div>
                @endif
                ...
                @if($avrPrev !== null)
                    <div class="flex items-center gap-2 text-xs text-gray-500 bg-white/5 px-4 py-2 rounded-xl border border-white/5">
                        ...
                        @if($wordCountPrev > $avrPrev)
                            <span class="text-emerald-400 font-bold">{{ $wordCountPrev }} &gt; {{ $avrPrev }}</span>
                            <span class="text-emerald-400/70">(Supera el promedio)</span>
                        @elseif($wordCountPrev === $avrPrev)
                            <span class="text-blue-400 font-bold">{{ $wordCountPrev }} = {{ $avrPrev }}</span>
                            <span class="text-blue-400/70">(En el promedio)</span>
                        @else
                            <span class="text-amber-400 font-bold">{{ $wordCountPrev }} &lt; {{ $avrPrev }}</span>
                            <span class="text-amber-400/70">(Debajo del promedio)</span>
                        @endif
                    </div>
                @endif
            </div>
