...
<div class="fade-in" x-data="{
    ...
    localStorage.setItem('peducativo-view-mode', val);
    window.dispatchEvent(new CustomEvent('peducativo-view-mode-changed', { detail: { mode: val } }))
})">
    <!-- Header -->
    ...
    <div x-cloak x-show="mode === 'grid'"
        ...
        <style>
            .peducativo-grid { display: grid; gap: 0.75rem; grid-template-columns: 1fr; }
            @media (min-width: 640px)  { .peducativo-grid { grid-template-columns: repeat(2, 1fr); } }
            @media (min-width: 1024px) { .peducativo-grid { grid-template-columns: repeat(3, 1fr); } }
            @media (min-width: 1280px) { .peducativo-grid { grid-template-columns: repeat(4, 1fr); } }
            .peducativo-empty { text-align: center; }
        </style>

        @forelse($peducativos as $peducativo)
            @if($loop->first)
            <div class="peducativo-grid">
            @endif
                <div class="bg-gray-900/40 backdrop-blur-md border border-white/5 rounded-lg p-4 hover:border-emerald-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/5 h-full flex flex-col">
                        <div class="flex items-start justify-between mb-3">
                            <div class="w-10 h-10 bg-cyan-500/10 rounded-lg flex items-center justify-center shrink-0">
                        ...
                        <h3 class="text-sm font-bold text-white mb-1 leading-snug">{{ $peducativo->name }}</h3>
                        @if($peducativo->description)
                            <p class="text-xs text-gray-400 mb-2">{{ Str::limit($peducativo->description, 60) }}</p>
                        @endif
                        <div class="flex flex-wrap gap-1.5 mb-3">
                            <span class="px-2 py-0.5 bg-purple-500/10 text-purple-400 text-[10px] rounded-full font-medium">{{ $peducativo->pescolar?->name ?? '—' }}</span>
                            <span class="px-2 py-0.5 bg-amber-500/10 text-amber-400 text-[10px] rounded-full font-medium">{{ $peducativo->pestudios_count }} plan(es)</span>
                        </div>
                        <div class="mt-auto pt-3 border-t border-white/5 flex items-center justify-between">
                            <div class="text-[10px] text-gray-500">
                                ...
                                @endif
                                @if($peducativo->manager)
                                    <span class="block text-gray-600">{{ $peducativo->manager->username }}</span>
                                @endif
                            </div>
        ...
        @if($peducativos->hasPages())
            <x-pagination-wrapper :paginator="$peducativos" />
        @endif
    </div>{{-- /grid view --}}
