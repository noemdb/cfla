<div class="fade-in">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-white mb-2">Pensums</h1>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-white/5">
                        ...
                        <th class="text-left px-4 py-3.5 text-[10px] font-bold uppercase tracking-widest text-gray-500">Plan Estudio</th>
                        <th class="text-left px-4 py-3.5 text-[10px] font-bold uppercase tracking-widest text-gray-500">Grado</th>
                        <th class="text-left px-4 py-3.5 text-[10px] font-bold uppercase tracking-widest text-gray-500">Asignatura</th>
                        <th class="text-center px-4 py-3.5 text-[10px] font-bold uppercase tracking-widest text-gray-500 hidden lg:table-cell">Comp.</th>
                        <th class="text-center px-4 py-3.5 text-[10px] font-bold uppercase tracking-widest text-gray-500 hidden lg:table-cell">Activo</th>
                        <th class="text-center px-4 py-3.5 text-[10px] font-bold uppercase tracking-widest text-gray-500 hidden lg:table-cell">Cargas</th>
    ...
    <!-- ===== MODAL: Vista Previa ===== -->
    <x-modal title="Detalles del Pensum" blur="lg" wire:model="previewMode" max-width="2xl">
        @if($previewPensum)
    ...
    <x-modal-card title="{{ $isEditing ? 'Editar Pensum' : 'Nuevo Pensum' }}" blur="lg" wire:model="modeForm" max-width="4xl" persistent>
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select wire:model="asignatura_id"
                            class="w-full bg-white/5 border border-white/10 text-gray-300 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 outline-none transition-all">
                            <option value="">Seleccione...</option>
                        ...
                        @if(empty($asignaturas) && $pestudio_id)
                            <p class="text-amber-400 text-[10px] mt-1">No hay asignaturas para este plan.</p>
                        @endif
                        @error('asignatura_id') <p class="text-red-400 text-[10px] mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-white/5 pt-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select wire:model="status_component"
                            class="w-full bg-white/5 border border-white/10 text-gray-300 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 outline-none transition-all">
                            <option value="false">No</option>
                            <option value="true">Sí</option>
                        </select>
                        @error('status_component') <p class="text-red-400 text-[10px] mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        <label class="flex items-center gap-2 cursor-pointer group mt-2">
                            <input type="checkbox" wire:model="status_active" value="1"
                    ...
                    <div>
                        ...
                        <label class="flex items-center gap-2 cursor-pointer group mt-2">
                            <input type="checkbox" wire:model="status_active_diagnostic" value="1"
                                {{ $status_active_diagnostic ? 'checked' : '' }}
                                class="w-4 h-4 text-emerald-500 bg-white/5 border-white/10 rounded focus:ring-emerald-500/50 focus:ring-2">
                            <span class="text-xs text-gray-300 group-hover:text-white transition-colors">Activo para diagnóstico</span>
                        </label>
                        @error('status_active_diagnostic') <p class="text-red-400 text-[10px] mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-white/5 pt-5">
                ...
                <div>
                    <input type="text" wire:model="observations" placeholder="Observaciones adicionales del pensum"
                        class="w-full bg-white/5 border border-white/10 text-gray-300 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 outline-none transition-all placeholder:text-gray-600">
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-x-4">
                ...
                <x-button primary label="{{ $isEditing ? 'Actualizar Pensum' : 'Guardar Pensum' }}" wire:click="save" spinner="save" />
            </div>
        </x-slot>
    </x-modal-card>
</div>
