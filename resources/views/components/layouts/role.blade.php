...
@php
    $resolvedLayout = $layout ?? \App\Services\MenuBuilder::resolveLayoutForUser();
    $resolvedSubtitle = $subtitle ?? \App\Services\MenuBuilder::subtitleForLayout($resolvedLayout);
@endphp
...
<body
    ...
    {{-- Navbar compartido por rol (resuelve layout/subtítulo desde el usuario) --}}
    <x-role-navbar :subtitle="$resolvedSubtitle" :layout="$resolvedLayout">
        <x-slot:navbarInfo>
            @yield('navbar-info')
        </x-slot:navbarInfo>
    </x-role-navbar>
        ...
        <div class="container mx-auto px-4 py-6">
            <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 text-center md:text-left">
                ...
                <div class="flex space-x-6 text-xs text-emerald-500/60 font-medium">
                    <span>{{ $footerLabel ?? $resolvedSubtitle }}</span>
