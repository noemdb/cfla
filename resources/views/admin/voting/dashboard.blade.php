            ...
            <div class="space-y-4">
                ...
                @if ($polls->isEmpty())
                    <div class="bg-white rounded-lg shadow p-12 text-center">
                        ...
                        <a href="{{ route('admin.voting.polls.create') }}"
                            class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors w-full sm:w-auto justify-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                            </svg>
                                ...
                                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                                    <div class="flex-1">
                                        <div class="flex flex-wrap items-center gap-2 mb-2">
                                            ...
                                            <span
                                                class="px-2 py-1 text-xs font-medium rounded-full {{ $poll->enable ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                {{ $poll->enable ? 'Activa' : 'Inactiva' }}
                                            </span>
                                        </div>
                                        <div class="flex flex-wrap gap-4 text-sm text-gray-600 mb-2">
                                            <span>🔗 Token: {{ $poll->access_token }}</span>
                                            <span>⏱️ Duración: {{ $poll->time_active }} min</span>
                                    ...
                                    <div class="flex flex-wrap sm:flex-nowrap gap-2">
                                        @if ($poll->enable)
                                            <form action="{{ route('admin.voting.polls.stop', $poll) }}" method="POST">
                                                @csrf
                                                <button type="submit"
                                                    class="w-full sm:w-auto px-3 py-1 text-sm bg-red-600 text-white rounded hover:bg-red-700 transition-colors">
                                                    ⏹️ Detener
                                                </button>
                                            </form>
                                        @else
                                            <form action="{{ route('admin.voting.polls.start', $poll) }}" method="POST">
                                                @csrf
                                                <button type="submit"
                                                    class="w-full sm:w-auto px-3 py-1 text-sm bg-green-600 text-white rounded hover:bg-green-700 transition-colors">
                                                    ▶️ Iniciar
                                                </button>
                                            </form>
                                        @endif
                                        ...
                                        <a href="{{ route('admin.voting.polls.edit', $poll) }}"
                                            class="sm:w-auto px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors text-center">
                                            ✏️ Editar
                                        </a>
                                        ...
                                        <form action="{{ route('admin.voting.polls.reset', $poll) }}" method="POST"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="w-full sm:w-auto px-3 py-1 text-sm bg-yellow-600 text-white rounded hover:bg-yellow-700 transition-colors">
                                                🔄 Reset
                                            </button>
                                        </form>
                                        ...
                                        <form action="{{ route('admin.voting.polls.destroy', $poll) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="w-full sm:w-auto px-3 py-1 text-sm bg-gray-600 text-white rounded hover:bg-gray-700 transition-colors">
                                                🗑️ Eliminar
                                            </button>
                                        </form>
                                    </div>
                                ...
                                <div class="px-6 pb-6">
                                    <div class="bg-blue-50 p-3 rounded-lg overflow-x-auto">
                                        <p class="text-sm text-blue-800 font-medium">
                                            🔗 Enlace público:
                                            <code class="ml-2 bg-white px-2 py-1 rounded text-xs break-all">
