                ...
                <div class="flex items-center">
                    ...
                    <div>
                        <p class="text-sm font-medium text-blue-800">
                            Token de acceso: <code class="bg-white px-2 py-1 rounded text-xs">{{ $poll->access_token }}</code>
                        </p>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                    <div class="flex items-center">
                        ...
                        <p class="text-sm text-yellow-800">
                            <strong>Advertencia:</strong> Esta encuesta está activa. Los cambios pueden afectar las votaciones en curso.
                        </p>
                    </div>
                </div>
            @endif
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="time_active"
                                name="time_active"
                                value="{{ old('time_active', $poll->time_active) }}"
                                min="1"
                                max="10080"
                    ...
                    <div id="options-container" class="space-y-4">
                        @foreach($poll->options as $index => $option)
                            <div class="option-row flex gap-2">
                                <div class="flex-1">
                                    <label for="option-{{ $index }}" class="block text-sm font-medium text-gray-700 mb-1">
                                        Opción {{ $index + 1 }}
                                        @if($option->votes_count > 0)
                                            <span class="text-xs text-gray-500">({{ $option->votes_count }} votos)</span>
                                        @endif
                                    </label>
                                    <input
                                        type="text"
...
document.querySelector('form').addEventListener('submit', function(e) {
    if (!confirm('¿Estás seguro? Esta acción eliminará todos los votos existentes ({{ $poll->votes_count }} votos).')) {
        e.preventDefault();
    }
});
@endif
