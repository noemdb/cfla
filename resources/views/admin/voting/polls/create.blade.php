        ...
        <form action="{{ route('admin.voting.polls.store') }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title') }}"
                            class="w-full bg-gray-800/50 border border-white/10 text-white rounded-lg px-5 py-4 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all outline-none placeholder:text-gray-600 @error('title') border-red-500/50 @enderror"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input type="number" id="time_active" name="time_active" value="{{ old('time_active', 60) }}"
                                min="1" max="10080"
                                class="w-full bg-gray-800/50 border border-white/10 text-white rounded-lg px-5 py-4 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all outline-none placeholder:text-gray-600 @error('time_active') border-red-500/50 @enderror"
                                required>
            ...
            <div
                                ...
                                <div class="flex-1 space-y-2">
                                    <div class="relative">
                                        ...
                                        <input type="text" name="options[{{ $i }}][label]"
                                            ...
                                            class="w-full bg-white/5 border border-white/10 text-white rounded-lg pl-10 pr-5 py-4 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all outline-none placeholder:text-gray-700"
                                            placeholder="Introduce la opción..." required>
                                    </div>
                                </div>
                                @if ($i >= 2)
                                    <button type="button" onclick="removeOption(this)"
                                        class="p-4 text-red-400 hover:text-white bg-red-500/10 hover:bg-red-500 rounded-lg border border-red-500/20 transition-all duration-300 group/del">
                ...
                <button type="button" onclick="addOption()"
                    class="w-full flex items-center justify-center gap-2 px-6 py-4 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white rounded-lg border border-white/5 border-dashed transition-all duration-300 font-bold text-sm">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="flex flex-col sm:flex-row gap-4 pt-4">
                ...
                <a href="{{ route('admin.voting.dashboard') }}"
                    class="px-8 py-4 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white rounded-lg border border-white/10 transition-all duration-300 font-bold uppercase tracking-widest text-sm text-center">
                    Cancelar
                </a>
            </div>
        </form>
        ...
        function addOption() {
            ...
            optionRow.innerHTML = `
                <div class="flex gap-3">
                    ...
                    <button type="button" onclick="removeOption(this)" class="p-4 text-red-400 hover:text-white bg-red-500/10 hover:bg-red-500 rounded-lg border border-red-500/20 transition-all duration-300 group/del">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
