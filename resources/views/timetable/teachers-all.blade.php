...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .schedule-block + .schedule-block{border-top:3px solid #0d9488;margin-top:12px;}
        .pestudio-inline{display:flex;flex-wrap:wrap;align-items:baseline;gap:6px;padding:6px 16px;background:#ecfdf5;border-left:3px solid #0d9488;border-bottom:1px solid #e5e7eb;}
        .pestudio-name{font-size:0.8rem;font-weight:800;color:#0f766e;}
        .pestudio-meta{font-size:0.68rem;color:#64748b;}
        ...
        td{border-bottom:1px solid #94a3b8;padding:8px;vertical-align:top;font-size:0.85rem;}
        ...
        @media print{
            ...
            .schedule-block + .schedule-block{border-top:2px solid #0d9488;margin-top:5px;}
            .pestudio-inline{padding:2px 10px;gap:5px;}
            .pestudio-name{font-size:0.62rem;}
            .pestudio-meta{font-size:0.55rem;}
            ...
            td.time{width:38px;font-size:0.64rem;}
    ...
    <div class="wrap">
        ...
        @forelse ($sheets as $sheet)
            ...
            <div class="card">
                ...
                {{-- Horarios del docente agrupados por P.Estudio, consecutivos. --}}
                @foreach ($teacherData['pestudios'] as $pestudioData)
                    @php($pestudio = $pestudioData['pestudio'])
                    @php($calendar = $pestudioData['calendar'])
                    <div class="schedule-block">
                        <div class="pestudio-inline">
                            <span class="pestudio-name">{{ $pestudio?->name ?? 'P.Estudio' }}</span>
                            <span class="pestudio-meta">
                                {{ $calendar->name }}
                                · Versión: {{ $calendar->version ?? '—' }}
                                · Creado: {{ $calendar->created_at?->format('d/m/Y H:i') ?? '—' }}
                                · Actualizado: {{ $calendar->updated_at?->format('d/m/Y H:i') ?? '—' }}
                            </span>
                        </div>

                    @foreach ($pestudioData['shifts'] as $shiftSchedule)
