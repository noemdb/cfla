...
| 2026-08-08 | **Fix: labels Mermaid sin espacios** (ej. "AutoconocimientoVocacioal") — nuevo requisito estricto "ESPACIADO DE PALABRAS" en los system prompts de `generateSlideDiagram`, `generateEmbedCard` y `repairMermaidBlock` (ejemplos correcto/incorrecto, división entre palabras completas). Además, defensa determinista: `validateMermaidDiagram` detecta runs de 23+ caracteres sin espacio en labels y dispara el reintento con feedback (mismo flujo de `diagramCorrectionBlock`). Test de regresión con ReflectionMethod. | `LessonWizard.php`, `LessonWizardRepairBlockTest.php` | ✅ commiteable (nuevo) |
