...
| D3 · "Siguiente lección" sticky | ✅ | `student-home.blade.php`: el hero ya tenía la próxima lección en su CTA; D3 añade una **mini-barra sticky** que la mantiene visible al hacer scroll. Estado Alpine en la raíz (`nextOpen`), `x-ref="heroSection"` en el hero. **FIX (5):** los listeners de scroll/resize van manuales en `init()` con cleanup en `destroy()` (mismo patrón probado que `readingNav` en activity-view), NO con el decorativo `@scroll.window.passive="updateNext()"`: el decorativo no disparaba de forma fiable entre los remounts de Alpine que hace Livewire, y lo tapaba el `wire:poll.10s` (re-ejecutaba `updateNext` cada 10s); retirado el poll, la barra no aparecía al hacer scroll. `init()` se re-ejecuta en cada setup de Alpine y `destroy()` limpia listeners en cada teardown (sin fugas entre morphs). La barra aparece cuando `hero.getBoundingClientRect().bottom <= 56` (56px = alto del navbar `h-14`: el CTA del hero se oculta detrás del navbar justo cuando la barra entra — ni hueco entre 0 y 56, ni CTA y barra a la vez). Reutiliza el color de materia D2 (`$__sc`/`$__scKey`) para el punto y la etiqueta; botón compacto "Continuar" con el mismo destino que el CTA del hero. `x-show` + `x-cloak` (al ocultarse no hay margen que aporte reflow al `space-y-8`), `x-transition` enter 200ms / leave 150ms (se anula sola bajo `prefers-reduced-motion`, bloque `<style>` de la vista), `sticky top-14 z-20` (debajo del navbar `z-30`), `bg-white/95 backdrop-blur` para leer sobre el scroll. **Guardia de timing:** el `init()` de la raíz corre antes de que los `x-ref` hijos se registren, así `updateNext()` arranca con `!!hero && ...` (cae a oculto, el default seguro) y el listener manual se autocorrige en cada scroll. Test: 1 nuevo — `test_home_renders_sticky_next_lesson_bar` (ver Validación D3). **Pulido posterior:** la barra pasó de full-bleed a **flotante full-width** — rompe fuera del `max-w-4xl` hacia el ancho real de la página (`-mx-[calc((100vw-100%)/2)]` estira la caja a 100vw, el mismo patrón que el navbar) y `px-[calc((100vw-100%)/2)]` realinea su contenido al eje de la columna (el `%` de margin/padding resuelve contra el content-box del contenedor, así que barra=100vw y el contenido interior coincide con el inset del navbar). Vidrio `bg-white/80 dark:bg-gray-800/80 backdrop-blur-md` + `shadow-lg` + `border-y` (no `border`: a 100vw las líneas laterales caerían sobre los bordes de pantalla; `border` desaparece) y `!mt-2` como hueco de vuelo de 8px bajo el navbar (`!` necesario: el `space-y-8` del contenedor, selector `> :not([hidden]) ~ :not([hidden])` con especificidad (0,2,0), vence al `.mt-2` plano (0,1,0)). `sticky top-14 z-20` intacto → el test D3 sigue verde. **Fix posterior (FIX 3):** `100vw` incluye el scrollbar vertical (~15px) → al aparecer la barra el exceso desbordaba el viewport y nacía un scrollbar horizontal. Se mide el ancho seguro en Alpine (`vw = document.documentElement.clientWidth`, `measureVw()` + `@resize.window`) y se usa como CSS var `--bar-vw`: `-mx-[calc((var(--bar-vw)-100%)/2)] px-[calc((var(--bar-vw)-100%)/2)]` con `:style="'--bar-vw: ' + vw + 'px'"` |
...
| H1 · NavTabs estudiante | ✅ | `student-home.blade.php`: barra de pestañas visible entre las stats y el primer panel — 4 tabs **Continuar / Lecciones / Distribución / Actividad** siguiendo el patrón reactivo del listado del profesor (`pevaluacion-list`): `nav` con `border-b border-gray-200 dark:border-gray-700`, botones `flex-1 ... border-b-2` que marcan la activa vía `:class` (emerald `text-emerald-600 dark:text-emerald-400 border-emerald-500 bg-emerald-500/5` vs gris `text-gray-500 dark:text-gray-400 border-transparent hover:text-gray-600 dark:hover:text-gray-300`). Cada pestaña es `<button role="tab" aria-controls="panel-*">`; cada sección vive en un panel `<div role="tabpanel" x-show="activeTab === '...'">`. **Decisiones clave:** (a) los paneles van con `x-show` (NO `@if`) para mantener todo el contenido en el DOM — los tests del home cortan el HTML por substring (strpos "Todas las Lecciones", fallback, etc.) y conservan el estado Alpine entre cambios; (b) el estado vive en Alpine con el binding bidireccional `@entangle('activeTab').live` (ver FIX (4)) y se persiste en `$queryString` (deep-link `?activeTab=...` mantiene la sección al refrescar); por eso la vinculación es `@entangle` en vez de re-seed + poll. (d) **Responsive (2026-08-08):** en pantallas `< sm` los labels ("Continuar", "Lecciones", ...) se ocultan con `<span class="hidden sm:inline ml-1">` y quedan solo los iconos (`svg w-3.5 h-3.5 inline -mt-0.5`, sin `mr-1` — el margen se movió al span); el texto sigue en el DOM, así que los tests de substring no se ven afectados. Mapeo: Continuar→secciones 2+3, Lecciones→4, Distribución→5, Actividad→6. Tests: 2 nuevos en `StudentHomeTest` (`test_home_renders_visible_navtabs_with_four_sections` — tablist + 4 tabs + paneles x-show con el binding `@entangle('activeTab').live`; `test_navtab_set_active_tab_persists_choice` — `setActiveTab` sincroniza y el queryString `activeTab` arranca en la pestaña elegida) |
...
> **Validación H1 (NavTabs):** `php8.2 artisan test --filter="StudentHomeTest|StudentAccessTest"` → **44 passed (191 assertions)** (baseline G: 42/172 → +2 tests / +19 assertions, sin regresiones). Los 2 tests nuevos verifican el sistema de pestañas completo: (a) la barra visible con `role="tablist"` y los 4 tabs con sus `aria-controls`, los 4 paneles `x-show` enlazados a `activeTab` y el binding bidireccional a Livewire (`@entangle('activeTab').live` + `@click` → `setActiveTab`); (b) que `setActiveTab` sincroniza el servidor y que el queryString `activeTab` (configurado en `$queryString`) arranca la pestaña elegida al recargar. Se usó `withQueryParams` para simular el deep-link. **FIX (4):** en la v1 la vinculación era siembra local (`activeTab: '{{ $activeTab }}'`) + `@this.setActiveTab(tab)`; tras el morph del cambio de pestaña Alpine volvía a sembrar el valor previo y el `:class` del tab activo quedaba desfasado un clic (1er clic cambiaba el contenido, 2º pintaba el borde emerald). Se sustituyó por `@entangle('activeTab').live`: Livewire↔Alpine quedan en sync en cada morph y el borde responde al 1er clic. **Nota de DOM:** los paneles van con `x-show` y no con `@if` porque los tests del home cortan el HTML por substring (strpos de "Todas las Lecciones", fallback "Publicaciones Recientes", etc.); un panel con `@if` vacío habría roto esos slices. Pint pasó sobre `StudentHomeTest.php`.
...
> **Validación D3:** `php8.2 artisan test --filter="StudentHomeTest|StudentAccessTest|AsignaturaColorKeyTest"` → **42 passed (157 assertions)** (baseline D2: 41/149 → +1 test / +8 assertions, sin regresiones); suite completa **320 passed (874 assertions)**, 1 fallo ajeno a D3 (el mismo `FlowDiagramTest.php:114` del hub en curso, no se toca). `test_home_renders_sticky_next_lesson_bar` nuevo: la barra existe y enlaza al detalle de la próxima lección (`route('student.lms.activity', $activity)`), arranca oculta (`x-show="nextOpen"` + `x-cloak`), es sticky bajo el navbar (`sticky top-14 z-20`), mide el hero (`x-ref="heroSection"`), reutiliza el color de materia D2 para el punto (`bg-rose-400` vía `Asignatura::colorKey('Test Asignatura')`→rose) y comparte el botón compacto "Continuar" con el CTA del hero. **Decisiones (revisión FIX (5)):** listeners de scroll/resize **manuales** en `init()` con cleanup en `destroy()` (mismo patrón que `readingNav` en activity-view); el decorativo `@scroll.window.passive` original no disparaba de forma fiable tras los remounts de Alpine de Livewire (lo tapaba el `wire:poll.10s` re-ejecutando `updateNext` cada 10s), así que retirado el poll la barra dejó de aparecer; los listeners manuales sobreviven y se limpian en cada teardown. Umbral `bottom <= 56` en vez de `<= 0` para que la barra entre exactamente cuando el CTA del hero queda tras el navbar (sin ventana sin CTA); guardia `!!hero && ...` en `updateNext()` porque el `init()` de la raíz corre antes de que los `x-ref` hijos se registren. Pint pass sobre `StudentHomeTest.php`.
