...
| **2026-08-08** | **Campo `content_type` en `lms_activity_sections` (branch `reclassify`, Spec en `blueprint/lesson`)** — Cambio estructural para identificar el tipo de contenido de cada sección sin inspeccionar sus contenidos: (1) columna `content_type` VARCHAR(30) NULL + índice, con backfill de las 359 secciones en la propia migración; (2) clasificación fina centralizada en `LmsContentClassifier` (`classifyContent` con precedencia mermaid>svg>math>html>markdown>text, `classifySection` con `mixed`/`none`); (3) `LmsActivityContentObserver` mantiene la caché al crear/editar/ocultar/eliminar contenidos; (4) comando `lms:sync-section-types` idempotente (reparación de drift); (5) integración aditiva: clase `section--{tipo}` en la vista print (reglas CSS por tipo), atributo `data-content-type` en `_content-renderer`. `null`/`mixed` → comportamiento idéntico al anterior. Distribución backfill: 292 text · 56 mixed · 5 svg · 3 math · 1 mermaid · 1 none · 1 html. 17 tests unitarios + 7 feature (observer/sync/accesor). | `database/migrations/2026_08_08_000001_add_content_type_to_lms_activity_sections.php` (nuevo), `app/Observers/LmsActivityContentObserver.php` (nuevo), `app/Console/Commands/SyncSectionTypes.php` (nuevo), `LmsContentClassifier.php`, `LmsActivitySection.php`, `AppServiceProvider.php`, `lessons-print.blade.php`, `_content-renderer.blade.php`, `tests/Unit/Lms/LmsContentClassifierTest.php`, `tests/Feature/Lms/SectionContentTypeTest.php` (nuevo) |
| **2026-08-08** | **Armonía tipográfica en contenido HTML etiquetado (`generateSlideHtmlTags`/`HtmlTaggingService`, Spec en `blueprint/lesson`)** — El HTML generado rompía la armonía (títulos text-3xl/2xl = 24–30px vs 14–17px del resto de la lección). Solución integral: (F0) `LmsDesignTokens` — escala única (título ≤ text-lg, stat ≤ text-2xl, p-4, shadow-sm) con `promptRules()` generado desde la constante; (F1/F2) rework del SYSTEM_PROMPT y USER_PROMPT: escala obligatoria, prohibición de duplicar el título de la sección, variantes de título unificadas y EJEMPLO COMPLETO corregido al nuevo scale; (F3) `LmsTypographyNormalizerService` — clamp determinista post-generación (solo baja: text-2xl+ → text-lg salvo patrón stat font-extrabold, p-5+ → p-4, shadow-md+ → shadow-sm, text-[NNpx]>18 → text-lg), enganchado en `tag()` como defensa en profundidad; (F4) el wizard etiqueta TODOS los bloques de la diapositiva (antes solo contents[0]). 9 tests del normalizador; 215 tests verdes. | `app/Services/Lms/LmsDesignTokens.php` (nuevo), `app/Services/Lms/LmsTypographyNormalizerService.php` (nuevo), `tests/Unit/Lms/LmsTypographyNormalizerServiceTest.php` (nuevo), `HtmlTaggingService.php`, `LessonWizard.php`, `blueprint/lesson/Spec - Armonia tipografica generateSlideHtmlTags.md` (nuevo) |
