...
class Catchment extends Model
{
    ...
    protected $fillable = [ 'group_id','token', 'firstname', 'lastname','ci_estudiant', 'grade', 'date_birth','status_foreign','country_foreign','status_siblings_college','brothers', 'gender','origin', 'representant_name', 'representant_lastname','representant_date_birth', 'representant_ci', 'email','day_appointment', 'relationship', 'occupation', 'educational_level', 'representant_phone', 'direction', 'reason_catholic', 'reason_interest', 'aspects_valued', 'expectations', 'importance_education', 'expectations_education', 'participation_activities', 'skills_talents', 'interests', 'challenges','status_active','status_accept_terms'];
    ...
    const COLUMN_COMMENTS = [
        ...
        'day_appointment' => 'Día de la cita',
    ...
    public static function getCountByAppointmentDay()
    {
        return self::selectRaw('day_appointment, COUNT(*) as total')
    ...
    public static function getCountByAppointmentDayAsArray()
    {
        return self::selectRaw('day_appointment, COUNT(*) as total')
            ->groupBy('day_appointment')
            ->orderBy('day_appointment')
            ->where('status_active',true)
            ->pluck('total', 'day_appointment');
    }

    public static function getCountByInterviewCreationDate()
    {
        return self::join('catchment_interviews', 'catchments.id', '=', 'catchment_interviews.catchment_id')
            ->selectRaw('DATE(catchment_interviews.created_at) as creation_date, COUNT(*) as total')
    ...
    public static function institutionOriginTotals($grado_id = null)
    {
        $catchments =  Catchment::select('catchments.id','catchments.origin', DB::raw('COUNT(*) AS total'))
            ...
            ->where('catchments.status_active',true)
            ;
