        ...
        <tr>
            <td style="width: 100%;">
                ...
                <p style="text-align: justify;">
                    Estimado(a) Sr(a).&nbsp;<em><strong>{{ ($profesor_guia) ? $profesor_guia->fullname : null}}</strong>, CI; {{ ($profesor_guia) ? $profesor_guia->ci_profesor : null}},</em>
                </p>

                <div style="text-align: justify; padding-left: 1rem; padding-botton: 1rem;margin-bottom: 1rem">
                    <span style="margin-bottom: 0rem">Profesor Guía de:</span>
                    <div style="padding-left: 1rem; margin-top: 0rem">
                        {{$grado->name ?? null}} {{$seccion->name ?? null}}
                    </div>
                </div>
                ...
                <p style="text-align: justify;">
                    Se le notifica que el docente [{{ ($profesor) ? $profesor->fullname : null}}], 
                    ha registrado una incidencia identificada con el código BE-I{{ $incident->id ?? null}},
                    realcionada al estudiante <span style="font-weight: bold;">{{$estudiant->fullname ?? null}}:
                </p>
                ...
                El único objetivo de este correo electrónico es enviar una notificación del registro de una incidencia realcionada a nuestra comunidad estudiantil.
                </p>

                <hr>

                <p style="text-align: center;">Atte.</p>
                <p style="text-align: center;">{{$profesor->fullname ?? null}} <br> {{$profesor->ci_profesor ?? null}}</p>
                <hr>
