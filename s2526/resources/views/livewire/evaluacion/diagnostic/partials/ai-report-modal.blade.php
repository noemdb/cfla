        ...
        <div class="modal-dialog modal-xl" role="document" style="max-width: 95% !important;">
            <div class="modal-content shadow-lg border-0">
                <div class="modal-header bg-dark text-white py-3">
                    ...
                    <button type="button" class="close text-white" wire:click="closeReportModal">
                ...
                <div class="modal-body p-4 bg-light" style="max-height: 85vh; overflow-y: auto;">
                    ...
                    @php
                         ...
                         * 1. Decode & safety
                         * ---------------------------------------------------------------- */
                        $draftRaw = $selectedReport->latestDraft->output_text ?? '{}';
                         ...
                         * 3. Areas
                         * ---------------------------------------------------------------- */
                        $aiAreas = is_array($draft['areas'] ?? null) ? $draft['areas'] : [];
                    ...
                    <script>
                        (function() {
                            var canvasId = 'profileChart';
                            var container = document.getElementById('div-' + canvasId);
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 mt-4 mt-lg-0 text-center">
                                    <div class="chart-container shadow-xs border"
                                        style="background: #f8f9fc; border-radius: 15px; padding: 20px;">
                                        <h6 class="font-weight-bold text-dark mb-4 small text-uppercase">Distribución
                                            de Precisión por Área</h6>
                                        <div id="div-profileChart"
                                ...
                                @php
                                    // Preparación de datos segura para Chart.js
                                    $labels = [];
                                    $data = [];
                                    $colors = [];
                                    $palette = [
                                    ...
                                    if (!empty($aiAreas) && is_array($aiAreas)) {
                                        foreach (array_values($aiAreas) as $i => $area) {
                                            $labels[] = $area['area_name'] ?? 'Área ' . ($i + 1);
                                            $data[] = $area['precision'] ?? 0;
                                            $colors[] = $palette[$i % count($palette)];
                                ...
                                <script>
                                    /**
                                     * Usamos un IIFE para no contaminar el scope global y asegurar 
                                     * que la gráfica se renderice correctamente en el modal.
                                     */
                                    (function() {
                                        var canvasId = 'profileChart';
                                        var containerId = 'div-' + canvasId;

                                        // 1. Limpiar canvas previo si existe (importante en modales)
                                        var oldCanvas = document.getElementById(canvasId);
                                        if (oldCanvas) {
                                            oldCanvas.remove();
                                        }

                                        // 2. Crear nuevo canvas
                                        var newCanvas = document.createElement('canvas');
                                        newCanvas.id = canvasId;
                                        ...
                                        new Chart(ctx, {
                                            ...
                                            data: {
                                                ...
                                                datasets: [{
                                                    ...
                                                    data: @json($data),
                                                    backgroundColor: @json($colors),
                                                    borderColor: @json($colors).map(c => c.replace('0.8', '1')),
                                                    borderWidth: 1
                                                }]
                                            },
                                            options: {
                                                ...
                                                tooltips: {
                                                    callbacks: {
                                                        label: function(tooltipItem) {
                                                            return tooltipItem.yLabel + '% de Precisión';
                                                        }
                                                    }
                                                }
                                            }
