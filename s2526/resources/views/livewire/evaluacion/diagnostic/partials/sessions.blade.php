        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">
                    ...
                    @if ($filterDiagMainId)
                        <div class="text-muted small">Diagnósticos:
                            {{ $diagMainCurrent->name ?? 'No hay diagnóstico seleccionado' }}</div>
                    @endif
                </h6>
                <div class="d-flex align-items-center">

                    <select wire:model.defer="selected_ai_service" class="form-control form-control-sm mr-2"
                        style="width: 150px;" title="Servicio IA">
                        <option value="deepseek">DeepSeek</option>
                        <option value="qwen">Qwen</option>
                        <option value="gemini">Gemini</option>
                        <option value="openrouter">OpenRouter</option>
                    </select>
                    <input type="text" wire:model.debounce.500ms="sessionsStudentNameFilter"
            ...
            <div class="card-body">
                {{-- Added validation to ensure $sessionsPaginated is a valid collection --}}
                @if (
                    $filterGradoId &&
                    ...
                    <?php
                    $selectedGrado = collect($grados)->firstWhere('id', $filterGradoId);
                    ...
                    <div class="alert alert-info alert-dismissible fade show mb-3" role="alert">
                        ...
                        <button type="button" class="close" wire:click="$set('filterGradoId', '')" aria-label="Close">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm" width="100%" cellspacing="0">
                        ...
                        <tbody class="small">
                            {{-- Added proper validation before foreach loop --}}
                            @if (isset($sessionsPaginated) && is_object($sessionsPaginated) && method_exists($sessionsPaginated, 'count'))
                                @forelse($sessionsPaginated as $studentData)
                                    <tr class="text-xs" wire:key="student-row-{{ $studentData->estudiant_id }}">
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="mr-2">
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                @if ($filterDiagMainId)
                                                    @if ($studentData->report)
                                                        <button type="button" class="btn btn-sm btn-success"
                                                            wire:click="viewAIReport({{ $studentData->estudiant_id }})"
                                                            title="Ver Informe IA (Generado el {{ $studentData->report->generated_at->format('d/m/Y') }})">
                                                            <i class="fas fa-file-alt mr-1"></i> Ver Informe
                                                        </button>
                                                        <button type="button"
                                                            class="btn btn-sm btn-outline-danger ml-1"
                                                            onclick="confirm('¿Estás seguro de eliminar este informe completamente? Esta acción no se puede deshacer.') || event.stopImmediatePropagation()"
                                                            wire:click="deleteAIReport({{ $studentData->estudiant_id }})"
                                                            title="Eliminar Informe (Dev)">
                                                            <i class="fas fa-trash"></i>
                                                        </button>
                                                    @else
                                                        <button type="button" class="btn btn-sm btn-outline-primary"
                                                            wire:click="generateAIReport({{ $studentData->estudiant_id }})"
                                                            wire:loading.attr="disabled" title="Generar Informe IA">
                                ...
                                @endforelse
                            @else
                                {{-- Added fallback when sessionsPaginated is not a valid collection --}}
                                <tr>
                                    <td colspan="6" class="text-center text-muted py-4">
                ...
                {{-- Added validation before pagination display --}}
                @if (isset($sessionsPaginated) && is_object($sessionsPaginated) && method_exists($sessionsPaginated, 'hasPages'))
                    {{-- Full Page Loading Overlay --}}
                    <div wire:loading.flex wire:target="generateAIReport"
                        style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255, 255, 255, 0.9); z-index: 9999; align-items: center; justify-content: center; flex-direction: column;">
                        <div class="text-center">
                            <i class="fas fa-robot fa-spin fa-4x text-primary mb-3"></i>
                            <h3 class="text-uppercase text-dark font-weight-bold">Generando contenido</h3>
                            <p class="text-muted lead">Por favor espere, procesando datos del estudiante...</p>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        ...
                        <div>
                            {{-- Replaced Laravel pagination links with Livewire pagination methods --}}
            ...
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800">
                            {{ $generalStats['total_sessions'] > 0 ? number_format(($generalStats['completed_sessions'] / $generalStats['total_sessions']) * 100, 1) : 0 }}%
                        </div>
                    </div>
                    <div class="col-auto">
...
@if ($showSessionDetailsModal && $selectedSession)
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-3" style="max-height: 80vh; overflow-y: auto;">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <p class="text-muted mb-2 small">
                                                {{ $selectedSession->estudiant->full_name ?? 'N/A' }}</p>

                                            <p class="mb-1 small"><strong>Identificación:</strong></p>
                                            <p class="text-muted mb-2 small">
                                                {{ $selectedSession->estudiant->ci_estudiant ?? 'N/A' }}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <p class="mb-1 small"><strong>Email:</strong></p>
                                            <p class="text-muted mb-2 small">
                                                {{ $selectedSession->estudiant->gsemail ?? ($selectedSession->estudiant->gsemail ?? 'N/A') }}
                                            </p>

                                            <p class="mb-1 small"><strong>Áreas de Formación:</strong></p>
                                            <p class="text-muted mb-2 small">
                                                {{ $selectedSession->unique_pensums ?? 0 }} área(s) activa(s)</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card-body text-center p-3">
                                    ...
                                    <div class="progress mb-2" style="height: 15px;">
                                        <div class="progress-bar bg-gradient-success"
                                            style="width: {{ $selectedSession->session_progress }}%">
                                            {{ $selectedSession->session_progress }}%
                                        </div>
                                    </div>
                                    <h5 class="text-success font-weight-bold mb-1">
                                        {{ $selectedSession->session_progress }}%</h5>
                                    <small class="text-muted">
                                        {{ $selectedSession->answered_questions }} de
                                        {{ $selectedSession->total_preguntas }} preguntas del grado
                                    </small>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row mb-2">
                        <div class="col-md-3">
                            ...
                            <div class="card border-left-info text-center">
                                <div class="card-body p-2">
                                    ...
                                    <h5 class="text-info font-weight-bold mb-0">{{ $selectedSession->total_sessions }}
                                    </h5>
                            ...
                            <div class="card border-left-success text-center">
                                <div class="card-body p-2">
                                    ...
                                    <h5 class="text-success font-weight-bold mb-0">
                                        {{ $selectedSession->completed_sessions }}</h5>
                            ...
                            <div class="card border-left-warning text-center">
                                <div class="card-body p-2">
                                    ...
                                    <h5 class="text-warning font-weight-bold mb-0">
                                        {{ $selectedSession->avg_duration_minutes ?? 0 }}</h5>
                        ...
                        <div class="col-md-3">
                            <div
                                class="card border-left-{{ $selectedSession->completed_sessions == $selectedSession->total_sessions ? 'success' : ($selectedSession->activo ? 'primary' : 'secondary') }} text-center">
                                <div class="card-body p-2">
                                    <i
                                        class="fas fa-{{ $selectedSession->completed_sessions == $selectedSession->total_sessions ? 'trophy' : ($selectedSession->activo ? 'play' : 'pause') }} text-{{ $selectedSession->completed_sessions == $selectedSession->total_sessions ? 'success' : ($selectedSession->activo ? 'primary' : 'secondary') }} fa-lg mb-1"></i>
                                    <h6
                                        class="text-{{ $selectedSession->completed_sessions == $selectedSession->total_sessions ? 'success' : ($selectedSession->activo ? 'primary' : 'secondary') }} font-weight-bold mb-0">
                                        {{ $selectedSession->completed_sessions == $selectedSession->total_sessions ? 'Completado' : ($selectedSession->activo ? 'En Progreso' : 'Pausado') }}
                                    </h6>
                    ...
                    @if (isset($selectedSession->pensum_stats) && count($selectedSession->pensum_stats) > 0)
                        ...
                        <div class="card mb-2">
                            ...
                            <div class="card-header bg-light py-2">
                                ...
                                <small class="text-muted">Mostrando solo áreas que el estudiante ha intentado
                                    ({{ count($selectedSession->pensum_stats) }} de
                                    {{ $selectedSession->total_areas_disponibles ?? 'N/A' }} disponibles)</small>
                            </div>
                            <!-- Reducido padding del body -->
                            <div class="card-body p-3">
                                @foreach ($selectedSession->pensum_stats as $pensumId => $stats)
                                ...
                                @if (isset($selectedSession->areas_pendientes) && $selectedSession->areas_pendientes > 0)
                                    <!-- Reducido padding del alert -->
                                    <div class="alert alert-info py-2 mb-0">
                                        <small>
                                            <i class="fas fa-info-circle mr-1"></i>
                                            <strong>{{ $selectedSession->areas_pendientes }}</strong> área(s) de
                                ...
                                <div class="col-md-6">
                                    <div class="mb-2">
                                        ...
                                        <p class="text-muted mb-0 small">
                                            {{ $selectedSession->iniciado_at ? \Carbon\Carbon::parse($selectedSession->iniciado_at)->format('d/m/Y H:i:s') : 'N/A' }}
                                        </p>
                                    </div>
                                    <div class="mb-2">
                                        <p class="mb-1 font-weight-bold text-info small">Última Actividad:</p>
                                        <p class="text-muted mb-0 small">
                                            {{ $selectedSession->last_session_date ? \Carbon\Carbon::parse($selectedSession->last_session_date)->format('d/m/Y H:i:s') : 'N/A' }}
                                        </p>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-2">
                                        ...
                                        <p class="text-muted mb-0 small">
                                            {{ $selectedSession->total_sessions > 0 ? round(($selectedSession->completed_sessions / $selectedSession->total_sessions) * 100, 1) : 0 }}%
                                            ({{ $selectedSession->completed_sessions }} de
                                            {{ $selectedSession->total_sessions }})
                                        </p>
                                    </div>
                                    @if ($selectedSession->completed_sessions == $selectedSession->total_sessions && $selectedSession->completado_at)
                                        <div class="mb-2">
                                            <p class="mb-1 font-weight-bold text-success small">Finalización Completa:
                                            </p>
                                            <p class="text-muted mb-0 small">
                                                {{ \Carbon\Carbon::parse($selectedSession->completado_at)->format('d/m/Y H:i:s') }}
                                            </p>
                                        </div>
                                    @endif
                                </div>
...
@if ($showSessionAnswersModal && $selectedSession)
    <div class="modal fade show" style="display: block;" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-xl" role="document" style="max-width: 95% !important;">
            <div class="modal-content">
                ...
                <!-- Body compacto con sistema de cards en lugar de accordion -->
                <div class="modal-body p-2" style="max-height: 75vh; overflow-y: auto;">
                    <div class="text-muted font-weight-bold">{{ $selectedSession->estudiant->full_name ?? 'N/A' }}
                    </div>
                    @if (count($selectedSessionAnswers) > 0)
                        <!-- Métricas compactas en cards pequeñas -->
                        <div class="row mb-3">
                            <div class="col-md-3">
                                    ...
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-md-3">
                                            <div class="progress" style="height: 15px;">
                                                <!-- Corrigiendo sintaxis de la clase CSS del progress-bar -->
                                        ...
                                        <div class="col-md-2 text-center">
                                            ...
                                            <br><small class="text-muted">Preguntas[Multiple]</small>
                                        </div>
