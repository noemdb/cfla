...
<div class="px-2">
    ...
    <div class="form-row">
        <div class="col-6">
            {!! Form::label('fecha', 'profesore', ['class' => 'm-0 font-weight-bold text-muted']) !!}
        ...
        <div class="col-2">
            ...
            <div class="btn-group btn-block" role="group" aria-label="Basic example">
                <button class="btn btn-dark disabled" type="button" id="btn_toprint">
            ...
            @foreach ($profesors as $profesor)
                ...
                <tr data-profesor_id="{{ $profesor->id ?? '' }}" data-id="{{ $profesor->id ?? '' }}"
                    ...
                    <td class="{{ $class_profesor ?? '' }}">
                        ...
                        <div class=" text-sm text-muted">{{ $profesor->ci_profesor }} <span class="text-muted">[Planes
                                de Evaluación:
                                {{ $pevaluacions->isNotEmpty() ? $pevaluacions->count() : null }}]</span></div>
                    </td>
                    ...
                    <td class="{{ $class_action ?? '' }}" id="btn-action-{{ $profesor->id }}">
                        ...
                        <div class="d-flex justify-content-start">
                            ...
                            @if ($incidents->count() > 0)
                                <a title="Agenda" class="btn btn-info btn-sm mr-1" href="#"
                                    wire:click="viewInterview({{ $profesor->id }})" role="button">
                                    <i class="{{ $icon_menus['interviews'] ?? '' }} fa-1x"></i>
                                </a>
                                <a title="Calendario" class="btn btn-light btn-sm mr-1" href="#"
                                    wire:click="viewCalendar({{ $profesor->id }})" role="button">
