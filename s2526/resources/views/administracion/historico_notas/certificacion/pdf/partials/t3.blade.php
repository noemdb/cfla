...
<table width="100%" cellpadding="0" cellspacing="0" border="1" id="table_normal">
    ...
    @foreach ($notas as $nota)
      @php
        ...
        $institucion = (!empty($nota->valor)) 
          ? ($arr_institucion[$nota->institucion_id] ?? '*') 
          : '*';
        ...
        $code = ($status_exonerate) ? $nota->tevaluacion->code : $code ;
        $mes = ($status_exonerate) ? substr($nota->fecha,0,2) : $mes ;
        $ano = ($status_exonerate) ? substr($nota->fecha,-4) : $ano ;
      ...
      @endphp
      <tr>
        <td align="left" class="{{ $class_asignatura_name ?? ''}} no_wrap">@admin {{$nota->pensum->id}} @endadmin {{ $nota->pensum->asignatura->name ?? ''}}</td>
        ...
        <td align="center" class="">{{ $code }} </td>
        <td align="center" class="">{{ $mes }} </td>
        <td align="center" class="">{{ $ano }} </td>
