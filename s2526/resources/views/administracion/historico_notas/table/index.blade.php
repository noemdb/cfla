            ...
            <tr data-asignatura="{{$asignatura->id}}" data-asignatura="{{$asignatura->id ?? ''}}" class="table-{{(empty($asignatura->administrativa->id)) ? 'default':'success'}}">
                ...
                <td class="{{ $class_action ?? '' }}" id="btn-action-{{ $asignatura->id }}">
                    {{-- <div class="btn-group" role="group" aria-label="Basic example"> --}}
                        <a title="Asignar Plan de Evaluación 1er Lapso" class="btn btn-primary btn-sm" href="{{ route('administracion.pevaluacions.create',['grado_id'=>$grado->id,'pensum_id'=>$pensum->id,'lapso_id'=>1]) }}" role="button">
                            1
                        </a>
                        <a title="Asignar Plan de Evaluación 2do Lapso" class="btn btn-success btn-sm" href="{{ route('administracion.pevaluacions.create',['grado_id'=>$grado->id,'pensum_id'=>$pensum->id,'lapso_id'=>2]) }}" role="button">
                            2
                            {{-- <i class="fas fa-list"></i> --}}
                        </a>
                        <a title="Asignar Plan de Evaluación 3er Lapso" class="btn btn-danger btn-sm" href="{{ route('administracion.pevaluacions.create',['grado_id'=>$grado->id,'pensum_id'=>$pensum->id,'lapso_id'=>3]) }}" role="button">
                            3
                        </a>
                    {{-- </div> --}}
                </td>
