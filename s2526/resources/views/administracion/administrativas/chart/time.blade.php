@php ($chart = ['range'=>1,'id_chart'=>'time_id_'.Auth::user()->id,'urlapi'=>route('administracion.administrativas.time.chart'),'tipo'=>'line','limit'=>10,'legend'=>true,'y_axes'=>'Cant. Estudiantes','x_axes'=>'Días'])
@section('scripts') @parent <script> requestData('{{ $chart['range'] }}','{{ $chart['id_chart'] }}','{{ $chart['urlapi'] }}','{{ $chart['tipo'] }}','{{ $chart['limit'] }}','{{ $chart['legend'] }}','{{ $chart['y_axes'] }}','{{ $chart['x_axes'] }}'); </script>@endsection

@component('administracion.elements.card.panel')
    @slot('class', 'dark')
    @slot('panelControls', 'true')
    @slot('id', $chart['id_chart'])
    @slot('header','Distribución diaria de la cantidad de inscripciones administrativas')
...
<div class="text-muted">
    En la gráfica se puede ver el total de inscripciones académicas realizadas diariamente
</div>
