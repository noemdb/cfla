...
<span class="dropdown-header text-center font-weight-bold text-dark bg-light bt-1 pb-1">Estructura de Cobro</span>
...
{{-- <a title="Crear deuda individual" class="{{ $class ?? 'nav-link' }} p-1 pl-2 text-dark" href="{{ route('administracion.configuraciones.concepto_pagos.index') }}">
    <i class="{{ $icon_menus['nuevo'] ?? '' }} text-primary"></i>
    Deuda Individual
</a> --}}
<a title="Planes Benéficos" class="{{ $class ?? 'nav-link' }} p-1 pl-2 text-dark disabled" href="{{ route('administracion.configuraciones.plan_beneficos.index') }}">
