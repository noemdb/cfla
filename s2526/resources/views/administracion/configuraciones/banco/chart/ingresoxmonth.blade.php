@php ($chart = ['range'=>'3','id_chart'=>'ingresoxmonth_id_'.Auth::user()->id,'urlapi'=>route('administracion.configuraciones.bancos.ingresoxmonth.chart',['banco_id'=>$banco->id]),'tipo'=>'line','limit'=>6 ])
@section('scripts')
    @parent
        ...
        @component('administracion.elements.canvas.chart')
            ...
            @slot('nav')                      
                <nav class="nav nav-tabs ranges" id="nav-tab" role="tablist" data-canvas="{{ $chart['id_chart'] ?? ''}}" data-urlapi="{{ $chart['urlapi'] ?? ''}}" data-tipo="{{ $chart['tipo'] ?? ''}}" data-limit="{{ $chart['limit'] ?? ''}}">
                    <a data-range="3" class="nav-item nav-link active" id="nav-3-tab" data-toggle="tab" href="#nav-3" role="tab" aria-controls="nav-3" aria-selected="false">3M</a>
