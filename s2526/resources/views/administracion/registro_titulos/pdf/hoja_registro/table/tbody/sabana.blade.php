<tbody>
    ...
    @foreach($estudiants as $estudiant)
        ...
        @php
            $decimal = 2;
            $promedio = ($lapso) ? $estudiant->getPromedioLapsoRound($lapso->id,$decimal) : $estudiant->getPromedioFinal($decimal) ;
        @endphp
        ...
        <tr style="font-size:0.62rem;">
            ...
            @endforeach
            <th style="text-align:center;">{{ (!empty($count_ar)) ? $count_ar:''}}</th>
    ...
    <tr style="background-color:#ccc;">
        ...
        @endforeach
        <th style="text-align:center;font-size:0.8rem">{{ (!empty($total_ar)) ? $total_ar:''}}</th>
        <th style="text-align:center;font-size:0.9rem">
            {{ $grado->getPorcAprobados($lapso_id) ?? '' }}
        </th>
    </tr>
