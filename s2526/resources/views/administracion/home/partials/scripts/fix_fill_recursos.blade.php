<div class=" h-100 border float-right w-25">
    <div class=" font-weight-bold text-center">
        Llenado de la tabla recursos
    </div>
    <div id="container"></div>
</div>

@section('scripts')
    @parent
    <script>
        $(document).ready(function () {
            var count = {{ $registro_pago_combinados->count() ?? 0 }}
            var start = 1;
            var size = count < 12 ? count : 12;
            fix_fill_recursos(start,size,count);
         });

        function fix_fill_recursos(start=1,size=12,count=12){
            var container = '#container'; //console.log(container);

            var ajaxurl = '{{ route("fix_fill_recursos", ["start"=>"_start_","size"=>"_size_"] )}}'; //console.log(ajaxurl);

            ajaxurl = ajaxurl.replace('_start_', start).replace('_size_', size); //console.log(ajaxurl);

            $.ajax({
                url: ajaxurl,
                type: "GET",
                success: function(data){

                    $(container).append(data);

                    start = start + size ;
                    if ( start < count ) {
                        size = (start+size) > count ? (count - start) : 12;
                        setTimeout ( fix_fill_recursos(start, size, count), 60000 );
                    }

                }
            });
        }
    </script>

@endsection
