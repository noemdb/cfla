...
{{-- <span>
    {{$registro_pago_combinado->ammount_pagado}}
</span> --}}

@foreach ($registropagos as $registropago)

    @php
        $creditos_aplicados = $registropago->all_credito_aplicados;
        $credito_generado = $registropago->all_credito_a_favor;
        $abonos_aplicados = $registropago->all_abono_aplicados;
        $descuentos_aplicados = $registropago->all_descuento_aplicado;
    @endphp
    ...
    <table width="100%" class="table table-striped table-hover p-1">
        ...
        <tbody id="tdatos" style="font-size:0.8rem; text-align:left">
            <tr>
                <td class="{{ $c_registro  ?? ''}}">
                ...
                <td class="{{ $c_conceptocancelados ?? '' }}">
                    @if ($registropago->conceptocancelados)
                        @php $conceptocancelados = $registropago->conceptocancelados; @endphp
