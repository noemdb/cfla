...
{{-- {!! Form::open(['route'=>'administracion.preinscripcions.update','method'=>'POST','id'=>'form-aprobe','class'=>'form-aprobe pb-2', 'role'=>'form-signin']) !!} --}}
...
<table width="100%" class="table table-striped table-hover table-sm small p-1" id="table-data-default">
    ...
    <thead>
        <tr>
            ...
            <th class="{{ $class_monto }} text-nowrap text-center">Errores</th>
    ...
    <tbody id="tdatos">
        ...
        @foreach ($preinscripcions as $prepago)
            ...
            <tr data-id="{{ $prepago->id ?? '' }}" id="tr_{{ $prepago->id ?? '' }}"
                ...
                {!! Form::model($prepago, [
                    'route' => ['administracion.preinscripcions.update', $prepago->id],
                    'method' => 'PUT',
                ...
                <td class="{{ $class_monto ?? '' }} text-nowrap">
                    {!! Form::text('ingreso_ammount_mask', $prepago->ingreso_ammount, [
                        $readonly,
                        'size' => '10',
                ...
                <td class="{{ $class_monto ?? '' }} text-wrap">
                    {{ $prepago->comment ?? '' }}
                    {!! Form::text('comment', $prepago->comment, ['size' => '10', 'placeholder' => 'Comentario', 'id' => 'comment']) !!}
                </td>
                ...
                <td class="{{ $class_monto ?? '' }} text-nowrap text-center">
                    ...
                    @if ($prepago->error_exist)
                        <span class="badge badge-danger" title="Número de referencia no encontrado">REF. N.E</span>
                        @php $status_error=true; @endphp
                    @else
                        @if ($prepago->error_apply)
                            <span class="badge badge-info" title="Referencia ya usada">REF. US.</span>
                            @php $status_error=true; @endphp
                        @else
                            @if ($prepago->error_date)
                                <span class="badge badge-warning" title="No coincide la fecha ingresada">FECHA
                                    N.C</span>
                                @php $status_error=true; @endphp
                            @endif
                            @if ($prepago->error_ammount)
                                <span class="badge badge-secondary" title="No coincide el monto ingresado">MONTO
                                    N.C</span>
                                @php $status_error=true; @endphp
                            @endif
                            @if ($prepago->error_bank)
                                <span class="badge badge-dark" title="No coincide el banco ingresado">BANCO N.C</span>
                                @php $status_error=true; @endphp
                            @endif
                        @endif
                    @endif
                ...
                <td class="{{ $class_action ?? '' }}">
                    ...
                    <div class="btn-group btn-group-sm">
                        ...
                        <fieldset id="fieldset_btn_{{ $prepago->id ?? '' }}" {{ $disabled ?? '' }}>
                            ...
                            <button title="Procesar notificación" type="button"
